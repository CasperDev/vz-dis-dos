...
SYS_MEMTOP_PTR    	EQU     $78B1   ; (WORD) Najwyższy dostępny adres RAM
...
SysRunBASICPrg		equ		$1ae8	; Start running BASIC Program (from address pushed on Stack)
...
; Bit 5        : Write Data (inverted, active LOW writes 1)
...
FLWR_PROTECT		equ		$13		; (read only) Write Protect Status (Bit 7) 1=Write Protected

;--------- Bits
FL_STEPPER_MASK	equ	%00001111		; Bit 0 -.Bit 3 :Stepper-motor control phases (active HIGH)
FL_DRV_1_ENABLE	equ	%00010000		; Bit 4 : Drive 1 enable.(active LOW)
FL_WRITE_DATA	equ	%00100000		; Bit 5 : Write Data Bit 
FL_WRITE_REQ	equ	%01000000		; Bit 6 : Write Request (active LOW)
FL_DRV_2_ENABLE	equ	%10000000		; Bit 7 : Drive 2 enable (active LOW)
...
;NAME BYTES OFFSET
	STRUCT DOSVTR
FILNO 	defb	0 				; IY+00 FILE#
FNAM	BLOCK	8," "			; IY+0l FILENAME
TYPE 	defw	0				; IY+09 FILE TYPE
DK 		defb	0 				; IY+0b SELECTED DRIVE# PATTERN
RQST 	defb	0				; IY+0c REQUEST CODE
SOURCE 	defb	0 				; IY+0d SOURCE DRIVE FOR DCOPY
UBFR 	defw	0				; IY+0e USER BUFFER ADDRESS
DESTIN 	defb	0				; IY+10 DEST DRIVE FOR DCOPY
SCTR 	defb	0 				; IY+11 USER SPEC. SECTOR NUMBER
TRCK 	defb	0				; IY+12 USER SPEC. TRACK NUMBER
RETRY 	defb	0				; IY+13 RETRY COUNT
DTRCK 	defb	0 				; IY+14 CURRENT TRACK NUMBER
NSCT 	defb	0				; IY+15 NEXT SCTR NUMBER
NTRK 	defb	0				; IY+16 NEXT TRK NUMBER
FCB1 	FCB		0				; IY+17 FILE CONTROL BLOCK 1 OPEN FLAG, STATUS, FNAM, TRK#, SCTR#, ENTRY IN SCTR
FCB2 	FCB		0 				; IY+22 FILE CONTROL BLOCK 2 OPEN FLAG, STATUS, FNAM, TRK#, SCTR#, ENTRY IN SCTR
DBFR 	defw	0				; IY+31 DATA BUFFER ADDRESS
LTHCPY 	defb	0				; IY+33 COPY OF LATCH
MAPADR 	defw	0				; IY+34 TRACK/SECTOR MAP ADDRESS
TRKCNT 	defb	0				; IY+36 TRK CNT FOR DCOPY
TRKPTR 	defb	0				; IY+37 TRK PTR FOR DCOPY
PHASE 	defb	0				; IY+38 STEPPER PHASE
	ENDS


FileNo 			equ		$00			; IY+00 FILE# (1 byte)

Filename		equ		$01			; IY+0l FILENAME (8 bytes)

FileType 		equ		$09			; IY+09 FILE TYPE (2 bytes)

SelectedDrive 	equ		$0b 		; IY+0b SELECTED DRIVE# PATTERN (1 byte)

RequestCode 	equ		$0c			; IY+0c REQUEST CODE (1 byte)

CopySrc 		equ		$0d			; IY+0d SOURCE DRIVE FOR DCOPY (1 byte)

CopyBuf 		equ		$0e			; IY+0e USER BUFFER ADDRESS (2 bytes)

CopyDst 		equ		$10			; IY+10 DEST DRIVE FOR DCOPY (1 byte)

UserSectorNo 	equ		$11			; IY+11 USER SPEC. SECTOR NUMBER (1 byte)

UserTrackNo 	equ		$12			; IY+12 USER SPEC. TRACK NUMBER (1 byte)

RetryCount 		equ		$13			; IY+13 RETRY COUNT (1 byte)

CurTrackNo 		equ		$14			; IY+14 CURRENT TRACK NUMBER (1 byte)

NextSectorNo 	equ		$15			; IY+15 NEXT SCTR NUMBER (1 byte)

NextTrackNo 	equ		$16			; IY+16 NEXT TRK NUMBER (1 byte)

FCBlock1 		equ		$17			; IY+17 FILE CONTROL BLOCK 1 [OPEN FLAG, STATUS, FNAM, TRK#, SCTR#, ENTRY IN SCTR (13 bytes)

FCBlock2 		equ		$24			; IY+24 FILE CONTROL BLOCK 2 [OPEN FLAG, STATUS, FNAM, TRK#, SCTR#, ENTRY IN SCTR (13 bytes)

DataBuf 		equ		$31			; IY+31 DATA BUFFER ADDRESS (2 bytes)

FLCtrlShadow 	equ		$33			; IY+33 COPY OF LATCH (1 byte)

MapAddress 		equ		$34			; IY+34 TRACK/SECTOR MAP ADDRESS (2 bytes)

CopyTrackCount 	equ		$36			; IY+36 TRK CNT FOR DCOPY (1 bytes)

CopyTrackPtr 	equ		$37			; IY+37 TRK PTR FOR DCOPY (1 bytes)

StepPhase 		equ		$38			; IY+38 STEPPER PHASE (1 byte) contains 4 bits Phase values duplicated in high and low nibble

SectorHeader	equ		$43			; IY+43 SECTOR HEADER (10 bytes) (ending sequence: GAP2 + IDAM ending)

SectorBuffer	equ		$4d			; IY+4d BUFFER DATA (154 bytes)

MapBuffer		equ		$e7			; IY+e7 MAP BUFFER
...
;***************************************************************************************************
	...
	jp PowerOn						; Disk power ON													;4008	c3 41 5f 	. A _ 
	jp PowerOff						; Disk power OFF												;400b	c3 52 5f 	. R _ 
	jp DOSError						; Error handling routine										;400e	c3 41 42 	. A B 
	jp ReadMap						; Read the track map of the disk								;4011	c3 17 47 	. . G 
	jp ClearSector					; Clear a sector of the disk									;4014	c3 49 47 	. I G 
	jp SaveMap						; Save the track map to the disk								;4017	c3 54 47 	. T G 
	jp DiskInit						; Initialize the disk											;401a	c3 08 4b 	. . K 
	jp CSI							; Command string interpreter									;401d	c3 67 53 	. g S 
	jp Asc2Hex						; Convert ASCII to HEX											;4020	c3 b9 53 	. . S 
	jp ReadIDAddressMark			; Read identification address mark								;4023	c3 ea 53 	. . S 
	jp CREATEDIRENTRY				; Create an entry in directory									;4026	c3 7b 58 	. { X 
	jp FINDEMPTYSEC					; Search for empty sector										;4029	c3 bf 58 	. . X 
	jp FINDFILE						; Search for file in directory									;402c	c3 13 59 	. . Y 
	jp FINDFREESPACE				; Search empty space in directory								;402f	c3 68 59 	. h Y 
	jp WRITESECTOR					; Write a sector to disk										;4032	c3 a1 59 	. . Y 
	jp READSECTOR					; Read a sector from disk										;4035	c3 27 5b 	. ' [ 
	jp DELAYMS						; Delay mS in reg C												;4038	c3 be 5e 	. . ^ 
	jp STEPIN						; Track step in													;403b	c3 ce 5e 	. . ^ 
	jp STEPOUT						; Track step out												;403e	c3 01 5f 	. . _ 
	jp LOADFILE						; Load a file from disk											;4041	c3 b1 43 	. . C 
	jp SAVEFILE						; Save a file to disk											;4044	c3 6e 44 	. n D 
...
; * Write Data = 1
	ld a,%01100001					; Write Req + Write Data + StepPhase (1)						;4047	3e 61 	> a 
...
;
	ld (iy+CurTrackNo),0			; Current Track Number = 0										;4087	fd 36 14 00 	. 6 . . 
	ld (iy+UserTrackNo),0			; User Track Number	= 0											;408b	fd 36 12 00 	. 6 . . 
	ld (iy+UserSectorNo),0			; User Sector Number = 0										;408f	fd 36 11 00 	. 6 . . 
	ld (iy+RetryCount),10			; Reset Retry Counter = 10										;4093	fd 36 13 0a 	. 6 . . 
	ld (iy+NextTrackNo),0			; Next Track = 0												;4097	fd 36 16 00 	. 6 . . 
	ld (iy+NextSectorNo),0			; Next Sector = 0												;409b	fd 36 15 00 	. 6 . . 
	ld (iy+FCBlock1),0				; File Control Block 1 Open Flag = not open 					;409f	fd 36 17 00 	. 6 . . 
	ld (iy+FCBlock2),0				; File Control Block 2 Open Flag = not open						;40a3	fd 36 24 00 	. 6 $ . 
	ld (iy+039h),000h				; TODO ;40a7	fd 36 39 00 	. 6 9 . 
	ld (iy+SelectedDrive),$10		; Selected Drive 1												;40ab	fd 36 0b 10 	. 6 . . 
	ld (iy+FLCtrlShadow),%01100001	; Update Shadow Register with value written to FDC Control 		;40af	fd 36 33 61 	. 6 3 a 
...
; this way futher rotation operations used by StepIn and StepOut don't need any corrections
	ld (iy+StepPhase),%00010001		; Stepper Phase 0001-0001										;40b3	fd 36 38 11 	. 6 8 . 

; -- set address of Sector Data as current operation Buffer
	...
	ld (iy+DataBuf),l				; store as Data Buffer (low byte)								;40be	fd 75 31 	. u 1 
	ld (iy+DataBuf+1),h				; store as Data Buffer (high byte)								;40c1	fd 74 32 	. t 2 

; -- fill Sector header data 
...
; --set address for Disk Map Buffer
	push iy							; copy IY register (DOS base) to HL								;40d1	fd e5 	. . 
	pop hl																							;40d3	e1 	. 
	ld de,MapBuffer					; offset from DOS base to Map Buffer							;40d4	11 e7 00 	. . . 
	add hl,de						; hl - absolute address of Map Buffer							;40d7	19 	. 
	ld (iy+MapAddress),l			; store low byte												;40d8	fd 75 34 	. u 4 
	ld (iy+MapAddress+1),h			; store high byte												;40db	fd 74 35 	. t 5 
...
; move Head 40 times so it will be on Track 00 no matter where it was when power off
	call PowerOn					; Disk power ON													;40de	cd 41 5f 	. A _ 
	ld b,40							; max 40 tracks to STEPOUT										;40e1	06 28 	. ( 
	call STEPOUT					; step out 40 times 											;40e3	cd 01 5f 	. . _ 
	call PowerOff					; Disk power OFF												;40e6	cd 52 5f 	. R _ 
...
; -- check if another ROM is present - chain initialization
...
; -- if match found then start ROM init 
...
; -- cleanup BASIC if  ???
	...
	ld a,(iy+039h)		;4242	fd 7e 39 	. ~ 9 
	or a			;4245	b7 	. 
	call nz,ClearBASIC		; no - Clear BASIC program and select drive D1							;4246	c4 44 51 	. D Q 
...
; -- turn DIsk Off and print error on screen
	call PowerOff					; Turn Off Disk power 											;426b	cd 52 5f 	. R _ 
...
; -- transfer control to BASIC/ROM simulating BREAK Key Pressed
	call PowerOff					; Disk power OFF												;428a	cd 52 5f 	. R _ 
...
; -- test if this code was called from specific ROM routine
	...
	ld hl,01d5bh					; expected return address										;4294	21 5b 1d 	! [ . 
...
; -- this code was calld from expected ROM routine
...
;***************************************************************************************************
; Parse and execute BASIC text expression 
...
.compareCmd:
	ld c,(hl)						; c - char to parse												;42f4	4e 	N 
...
; -- found start of command
	...
	cp c							; is equal to parsed char? 										;4301	b9 	. 
...
.compareNext:
	...
	ld a,(de)						; a - next char of Command										;4307	1a 	. 
...
ExecDOSCmd:
	...
	ex de,hl						; hl - next char after filename									;432a	eb 	. 
	inc hl							; hl - parse point												;432b	23 	# 
	push bc							; push routine on stack from bc									;432c	c5 	. 
...
; -- expected required name of file enclosed with double quote chars
	call CSI						; parse filename and copy it to (iy+filename)					;4391	cd 67 53 	. g S 
	push hl							; save hl - parse point											;4394	e5 	. 
	or a							; was any Error?												;4395	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;4396	c2 41 42 	. A B 

; -- set type of file as BASIC text ('T') and call DOS routine
	ld (iy+FileType),'T'			; set type of file as BASIC text ('T')							;4399	fd 36 09 54 	. 6 . T 
	call LOADFILE					; Load a file from disk											;439d	cd b1 43 	. . C 
	or a							; was any Error?												;43a0	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;43a1	c2 41 42 	. A B 
...
; -- push on Stack address of 1st BASIC line and run it
	...
	jp SysRunBASICPrg				; Start running BASIC Program ---------------------------------	;43ae	c3 e8 1a 	. . . 
...
; 16-bit addresses of start and end (excluded) of memory area where file must be loaded.
; IN: interrups disabled
;     (iy+Filename) - name of the file to load 
; OUT: a - Error code
...
; -- turn onn drive and wait 50 ms
	call PowerOn					; Disk power ON													;43b2	cd 41 5f 	. A _ 
	push bc							; save bc														;43b5	c5 	. 
	ld bc,50						; number of miliseconds to delay								;43b6	01 32 00 	. 2 . 
	call DELAYMS					; delay 50 ms													;43b9	cd be 5e 	. . ^ 
	pop bc							; restore bc													;43bc	c1 	. 
; -- try to find file on disk
	call FINDFILE					; Search for file in directory									;43bd	cd 13 59 	. . Y 
...
.verifyType:
; -- verify file types 
	ld a,(iy+FileType)				; a - requested file type										;43ca	fd 7e 09 	. ~ . 
	cp (iy+FileType+1)				; is the same as type of file just found?						;43cd	fd be 0a 	. . . 
	ld a,12							; a - Error 12 - FILE TYPE MISMATCH								;43d0	3e 0c 	> . 
	ret nz							; no - ------------------ End of Proc (with Error) ------------	;43d2	c0 	. 

; --------------------------------------------------------------------------------------------------
; When FINDFILE returns with Error 02 (FILE ALREADY EXISTS) it means we have found Directory Entry
...
; -- set track number of Sector to read
	...
	ld (iy+UserTrackNo),a			; set as requested Track Number									;43d5	fd 77 12 	. w . 
; -- set Sector Number of Sector to read
	...
	ld (iy+UserSectorNo),a			; set as requested Sector Number								;43da	fd 77 11 	. w . 
; -- set boundary of memory where to load file
	...
	ld d,(hl)						; de - address of memory where to load file						;43e0	56 	V 
...
; -- set destination address as BASIC program Start and DOS Buffer
	...
	ld (iy+CopyBuf),e				; store destination program start as buffer address				;43e6	fd 73 0e 	. s . 
	ld (iy+CopyBuf+1),d																				;43e9	fd 72 0f 	. r . 
...
.loadFileChunk:
	call READSECTOR					; Read a sector from disk										;43f3	cd 27 5b 	. ' [ 
	or a							; was it any Error?												;43f6	b7 	. 
	jp nz,.exitError				; yes - cleanup BASIC addresses and exit with Error				;43f7	c2 41 44 	. A D 

; -- no error 
	ld l,(iy+DataBuf)				; 																;43fa	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)				; hl - address of buffer with Sector data						;43fd	fd 66 32 	. f 2 
...
; -- setup track and sector number to read next
	...
	ld (iy+UserTrackNo),a			; set Track Number to read next									;4407	fd 77 12 	. w . 
	ld a,(hl)						; a - Number of Sector with next data chunk						;440a	7e 	~ 
	ld (iy+UserSectorNo),a			; set Sector Number to read next								;440b	fd 77 11 	. w . 
	pop hl							; restore hl - address of Sector data							;440e	e1 	. 

; -- copy program chunk from Sector Buffer to destination memory
	ld e,(iy+CopyBuf)																				;440f	fd 5e 0e 	. ^ . 
	ld d,(iy+CopyBuf+1)				; de - destination address to copy program chunk				;4412	fd 56 0f 	. V . 
	ld bc,126						; 126 bytes to copy	(full sector as default)					;4415	01 7e 00 	. ~ . 

; -- if next track and sector equals 0 than this is last sector
; -- size of data in last sector can less than 126 bytes 
	ld a,(iy+UserTrackNo)			; a - next Track Number											;4418	fd 7e 12 	. ~ . 
	or (iy+UserSectorNo)			; is track and sector numbers = 0?								;441b	fd b6 11 	. . . 
...
; -- update destination address for next program chunk
	ld (iy+CopyBuf),e				; store LSB of destination address								;4422	fd 73 0e 	. s . 
	ld (iy+CopyBuf+1),d				; store MSB of destination address								;4425	fd 72 0f 	. r . 
; -- check if it was last sector (Z=1) 
	ld a,(iy+UserTrackNo)			; a - next Track number											;4428	fd 7e 12 	. ~ . 
	or (iy+UserSectorNo)			; is track and sector numbers both are 0?						;442b	fd b6 11 	. . . 
...
; -- file loaded - turn off Drive and return with No Error
	call PowerOff					; Disk power OFF												;443c	cd 52 5f 	. R _ 
...
DCmdSAVE:
; -- expected required name of file enclosed with double quote chars
	call CSI						; parse filename and copy it to (iy+filename)					;444e	cd 67 53 	. g S 
	push hl							; save hl - parse point											;4451	e5 	. 
	or a							; was any Error?												;4452	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;4453	c2 41 42 	. A B 
; -- turn on Disk Drive and wait 2 ms
	call PowerOn					; Disk power ON													;4456	cd 41 5f 	. A _ 
	push bc							; save bc 														;4459	c5 	. 
	ld bc,2							; bc - 2 miliseconds to delay									;445a	01 02 00 	. . . 
	call DELAYMS					; delay 2 ms 													;445d	cd be 5e 	. . ^ 
	pop bc							; restore bc													;4460	c1 	. 
; -- check if Disk is not Write-Protected
	in a,(FLWR_PROTECT)				; a - read Write Protected flag from FDC						;4461	db 13 	. . 
	or a							; is bit 7 set? (write protected)								;4463	b7 	. 
	ld a,4							; a - Error 04 - DISK WRITE PROTECTED							;4464	3e 04 	> . 
	jp m,DOSError					; yes - go to Error handling routine --------------------------	;4466	fa 41 42 	. A B 
; -- set type of file as BASIC text ('T') and continue as DOS routine
	ld (iy+FileType),'T'			; set type of file as BASIC text ('T')							;4469	fd 36 09 54 	. 6 . T 
	pop hl							; restore hl - parse point										;446d	e1 	. 



;***************************************************************************************************
; Save program or data from block of memory sepcified by start address in (SYS_BASIC_START_PTR) 
; end end address in (SYS_BASIC_END_PTR) fo file specified by name in (iy+Filename) 
; and type in (iy+FileType).
; IN: (iy+Filename) - filename to save
;     (iy+FileType) - type of file 
...
; -- delay 50 ms
	...
	call DELAYMS					; delay 50 ms													;447e	cd be 5e 	. . ^ 
...
; -- setup address of Program as data source
	...
	ld (iy+CopyBuf),e				; store LSB of address											;4486	fd 73 0e 	. s . 
	ld (iy+CopyBuf+1),d				; set Program address as Copy Buffer (source)					;4489	fd 72 0f 	. r . 

; -- read Disk Map
	ld (iy+UserTrackNo),0			; Track 0 contains Sector with Disk Map							;448c	fd 36 12 00 	. 6 . . 
	ld (iy+UserSectorNo),15			; Sector 15 (on track 0) contains Disk Map						;4490	fd 36 11 0f 	. 6 . . 
	call READSECTOR					; Read Disk Map into Sector Buffer from disk					;4494	cd 27 5b 	. ' [ 
	or a							; was any Error?												;4497	b7 	. 
	jp nz,SF_ExitError				; yes -  exit with Error --------------------------------------	;4498	c2 9a 48 	. . H 

; -- no error - copy Disk Map from Sector Buffer to Map Buffer
	ld e,(iy+MapAddress)																			;449b	fd 5e 34 	. ^ 4 
	ld d,(iy+MapAddress+1)			; de - (dst) buffer for Disk Map								;449e	fd 56 35 	. V 5 
	ld l,(iy+DataBuf)																				;44a1	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)				; hl - (src) buffer with sector data							;44a4	fd 66 32 	. f 2 
	ld bc,80						; bc - number of bytes to copy									;44a7	01 50 00 	. P . 
	ldir							; copy Disk Map 												;44aa	ed b0 	. . 
; -- create Directory Entry for file
	call CREATEDIRENTRY				; Create an entry in directory									;44ac	cd 7b 58 	. { X 
	or a							; was any Error?												;44af	b7 	. 
	jp nz,SF_ExitError				; yes -  exit with Error --------------------------------------	;44b0	c2 9a 48 	. . H 

; -- Directory Entry was created for this file
; NextTrackNo and NextSectorNo contains values for sector alloacated for chunk od file data
; 

.writeNextChunk:
; -- check next Track and Sector - if 0 then last sector has been written
	ld a,(iy+NextTrackNo)			; a - track numer for first free sector							;44b3	fd 7e 16 	. ~ . 
	or (iy+NextSectorNo)			; track and sector numbers are 0? (no free sectors)				;44b6	fd b6 15 	. . . 
...
; -- save track and sector numbers allocated for 1st chunk of file
	ld d,(iy+NextTrackNo)			; d - Track Number for chunk of file 							;44bc	fd 56 16 	. V . 
	ld e,(iy+NextSectorNo)			; e - Sector Number for chunk of file 							;44bf	fd 5e 15 	. ^ . 

; -- check if there is free space on Disk for next sector
	call FINDEMPTYSEC				; search for empty sector in Disk Map							;44c2	cd bf 58 	. . X 
	cp 7							; was Error 07 - DISK FULL?										;44c5	fe 07 	. . 
	jr nz,.checkOtherError			; no - check if other error										;44c7	20 19 	  . 

; -- disk full 
	call FINDFILE					; Search for file in directory									;44c9	cd 13 59 	. . Y 
	cp 2							; is Error 02 - FILE ALREADY EXISTS?							;44cc	fe 02 	. . 
	ld a,6							; a - Error 6 - DISK I/O ERROR									;44ce	3e 06 	> . 
	jp nz,SF_ExitError				; no - exit with Error 06 -------------------------------------	;44d0	c2 9a 48 	. . H 
...
; -- disk full but file exists on Disk - mark it as deleted
	...
	call WRITESECTOR				; Write a sector with Directory to disk							;44da	cd a1 59 	. . Y 
	ld a,7							; a - Error 07 - DISK FULL										;44dd	3e 07 	> . 
	jp SF_ExitError					; exit with Error 07 ------------------------------------------	;44df	c3 9a 48 	. . H 

.checkOtherError:
	or a							; was any Error (after Find Empty Sector)?						;44e2	b7 	. 
	jp nz,SF_ExitError				; yes - exit with Error ---------------------------------------	;44e3	c2 9a 48 	. . H 

; -- setup destination sector to write file chunk
	ld (iy+UserTrackNo),d			; set Track Number												;44e6	fd 72 12 	. r . 
	ld (iy+UserSectorNo),e			; set Sector Number												;44e9	fd 73 11 	. s . 

; -- prepare sector buffer 
	ld l,(iy+DataBuf)				; 																;44ec	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)				; hl - address of Sector buffer									;44ef	fd 66 32 	. f 2 
...
; -- get address of data chunk 
	ld l,(iy+CopyBuf)				; hl - address of data chunk to write on Disk					;44fd	fd 6e 0e 	. n . 
	ld h,(iy+CopyBuf+1)				; 																;4500	fd 66 0f 	. f . 
...
; -- store start of next chunk 
	...
	ld (iy+CopyBuf),l				; store address of next chunk of data							;451a	fd 75 0e 	. u . 
	ld (iy+CopyBuf+1),h																				;451d	fd 74 0f 	. t . 
...
; -- store link - next Track and Sector Number into last 2 bytes of sector
	ld a,(iy+NextTrackNo)			; a - Track Number of sector for next chunk of file 			;4527	fd 7e 16 	. ~ . 
	ld (de),a						; store Track number as 127th byte of sector					;452a	12 	. 
	inc de							; de - points to 128th byte of sector							;452b	13 	. 
	ld a,(iy+NextSectorNo)			; a - Sector NUmber for next chunk of file						;452c	fd 7e 15 	. ~ . 
	ld (de),a						; store Sector NUmber as 128th byte of sector					;452f	12 	. 

.flushSector:
	call WRITESECTOR				; Write a sector to disk										;4530	cd a1 59 	. . Y 
	or a							; was any Error?												;4533	b7 	. 
	jp nz,SF_ExitError				; yes - exit with Error ---------------------------------------	;4534	c2 9a 48 	. . H 
...
; -- get Disk Map address 
	ld l,(iy+MapAddress)			; hl - address of Disk Map 										;453b	fd 6e 34 	. n 4 
	ld h,(iy+MapAddress+1)																			;453e	fd 66 35 	. f 5 
; -- calculate byte offset 
	ld a,(iy+NextTrackNo)			; a - Track Number of sector for next chunk of file				;4541	fd 7e 16 	. ~ . 
	...
	ld a,(iy+NextSectorNo)			; a - Sector number for next chunk of file						;454a	fd 7e 15 	. ~ . 
...
; -- set it was last chunk of file
	ld (iy+NextTrackNo),0			; set next track to 0 (no next sector)							;456c	fd 36 16 00 	. 6 . . 
	ld (iy+NextSectorNo),0			; set next sectro to 0 (no next sector)							;4570	fd 36 15 00 	. 6 . . 
...
.finalizeSave:
	call FINDFILE					; Search for file in directory									;457c	cd 13 59 	. . Y 
	cp 2							; was Error 2 - FILE ALREADY EXISTS								;457f	fe 02 	. . 
	jp nz,SF_ExitError				; no - exit with Error ----------------------------------------	;4581	c2 9a 48 	. . H 
...
; -- write directory entry to Disk
	call WRITESECTOR				; Write a sector with Directory to Disk							;4596	cd a1 59 	. . Y 
	or a							; was any Error?												;4599	b7 	. 
	jp nz,SF_ExitError				; yes - exit with Error ---------------------------------------	;459a	c2 9a 48 	. . H 

; -- update Disk Map sector

; -- clear data buffer (fill with 0 bytes)
	ld l,(iy+DataBuf)				; hl - address of data buffer									;459d	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)																				;45a0	fd 66 32 	. f 2 
...
; -- copy disk Map into data buffer
	...
	ld e,(iy+MapAddress)			; de - address of Disk Map data									;45af	fd 5e 34 	. ^ 4 
	ld d,(iy+MapAddress+1)																			;45b2	fd 56 35 	. V 5 
...
; -- write Disk Map into Disk
	ld (iy+UserTrackNo),0			; set Track Number 0											;45bb	fd 36 12 00 	. 6 . . 
	ld (iy+UserSectorNo),15			; set Sector Number 15											;45bf	fd 36 11 0f 	. 6 . . 
	call WRITESECTOR				; Write a sector to disk										;45c3	cd a1 59 	. . Y 
; -- turn off disk drive
	push af							; save a - error code after Write Sector						;45c6	f5 	. 
	call PowerOff					; Disk power OFF												;45c7	cd 52 5f 	. R _ 
...
; -- exit
	...
	jp nz,DOSError					; yes - go to Error handling routine							;45d7	c2 41 42 	. A B 
...
LoadAndRunFile:
	...
	jp nz,DOSError					; yes - goto Error handling routine	---------------------------	;45e0	c2 41 42 	. A B 
; -- valid filename detected
	ld (iy+FileType),'T'			; set File Type 'T' - BASIC text program						;45e3	fd 36 09 54 	. 6 . T 
	call LOADFILE					; Load a file from disk											;45e7	cd b1 43 	. . C 
	or a							; was any Error?												;45ea	b7 	. 
	jp nz,DOSError					; yes - goto Error handling routine	---------------------------	;45eb	c2 41 42 	. A B 
...
DCmdOPEN:	
	call 02828h		;45f5	cd 28 28 	. ( ( 
	call ParseFilename		;45f8	cd 78 53 	. x S 
	push hl			;45fb	e5 	. 
	or a			;45fc	b7 	. 
	jp nz,DOSError		; Error handling routine	;45fd	c2 41 42 	. A B 
	pop af			;4600	f1 	. 
	rst 8			;4601	cf 	. 
	inc l			;4602	2c 	, 
	call 02b1ch		;4603	cd 1c 2b 	. . + 
	or a			;4606	b7 	. 
	jr z,l460eh		;4607	28 05 	( . 
	cp 002h		;4609	fe 02 	. . 
	jp nc,01e4ah		;460b	d2 4a 1e 	. J . 
l460eh:
	ld (iy+RequestCode),a		;460e	fd 77 0c 	. w . 
	push hl			;4611	e5 	. 
	call sub_4778h		;4612	cd 78 47 	. x G 
	cp 005h		;4615	fe 05 	. . 
	jp nz,DOSError		; Error handling routine	;4617	c2 41 42 	. A B 
	push hl			;461a	e5 	. 
	call sub_4fa5h		;461b	cd a5 4f 	. . O 
	pop hl			;461e	e1 	. 
	ld (hl),001h		;461f	36 01 	6 . 
	inc hl			;4621	23 	# 
	ld a,(iy+RequestCode)		;4622	fd 7e 0c 	. ~ . 
	ld (hl),a			;4625	77 	w 
	inc hl			;4626	23 	# 
	push iy		;4627	fd e5 	. . 
	pop de			;4629	d1 	. 
	...
	push de			;4631	d5 	. 
	di			;4632	f3 	. 
	call PowerOn		; Disk power ON			;4633	cd 41 5f 	. A _ 
	push bc			;4636	c5 	. 
	ld bc,00032h		;4637	01 32 00 	. 2 . 
	call DELAYMS		; Delay mS in reg C								;463a	cd be 5e 	. . ^ 
	pop bc			;463d	c1 	. 
	call FINDFILE		; Search for file in directory					;463e	cd 13 59 	. . Y 
	cp 002h		;4641	fe 02 	. . 
	jp nz,l466bh		;4643	c2 6b 46 	. k F 
	ld a,(iy+FileType+1)		;4646	fd 7e 0a 	. ~ . 
	cp 044h		;4649	fe 44 	. D 
	ld a,00ch		;464b	3e 0c 	> . 
	jp nz,DOSError		; Error handling routine	;464d	c2 41 42 	. A B 
	pop hl			;4650	e1 	. 
	ld a,(de)			;4651	1a 	. 
	ld (hl),a			;4652	77 	w 
	ld (iy+UserTrackNo),a		;4653	fd 77 12 	. w . 
	inc de			;4656	13 	. 
	inc hl			;4657	23 	# 
	ld a,(de)			;4658	1a 	. 
	ld (hl),a			;4659	77 	w 
	ld (iy+UserSectorNo),a		;465a	fd 77 11 	. w . 
	xor a			;465d	af 	. 
	inc hl			;465e	23 	# 
	ld (hl),a			;465f	77 	w 
	ld a,(iy+RequestCode)		;4660	fd 7e 0c 	. ~ . 
	or a			;4663	b7 	. 
	jr nz,l46b9h		;4664	20 53 	  S 
	call PowerOff		; Disk power OFF		;4666	cd 52 5f 	. R _ 
	pop hl			;4669	e1 	. 
	ret			;466a	c9 	. 
l466bh:
	ld c,a			;466b	4f 	O 
	ld a,(iy+RequestCode)		;466c	fd 7e 0c 	. ~ . 
	or a			;466f	b7 	. 
	ld a,c			;4670	79 	y 
	jp z,l46a8h		;4671	ca a8 46 	. . F 
	in a,(FLWR_PROTECT)		;4674	db 13 	. . 
	or a			;4676	b7 	. 
	ld c,004h		;4677	0e 04 	. . 
	jp m,l46a8h		;4679	fa a8 46 	. . F 
	call ReadMap		; Read the track map of the disk				;467c	cd 17 47 	. . G 
	ld (iy+FileType),044h		;467f	fd 36 09 44 	. 6 . D 
	call CREATEDIRENTRY		; Create an entry in directory					;4683	cd 7b 58 	. { X 
	or a			;4686	b7 	. 
	jp nz,DOSError		; Error handling routine	;4687	c2 41 42 	. A B 
	pop hl			;468a	e1 	. 
	ld a,(iy+NextTrackNo)		;468b	fd 7e 16 	. ~ . 
	ld (iy+UserTrackNo),a		;468e	fd 77 12 	. w . 
	ld (hl),a			;4691	77 	w 
	inc hl			;4692	23 	# 
	ld a,(iy+NextSectorNo)		;4693	fd 7e 15 	. ~ . 
	ld (iy+UserSectorNo),a		;4696	fd 77 11 	. w . 
	ld (hl),a			;4699	77 	w 
	inc hl			;469a	23 	# 
	ld (hl),000h		;469b	36 00 	6 . 
	call ClearSector		; Clear a sector of the disk					;469d	cd 49 47 	. I G 
	call SaveMap		; Save the track map to the disk				;46a0	cd 54 47 	. T G 
	call PowerOff		; Disk power OFF		;46a3	cd 52 5f 	. R _ 
	pop hl			;46a6	e1 	. 
	ret			;46a7	c9 	. 
l46a8h:
	pop hl			;46a8	e1 	. 
	...
	ld a,c			;46af	79 	y 
	or a			;46b0	b7 	. 
	jp nz,DOSError		; Error handling routine	;46b1	c2 41 42 	. A B 
	ld a,00dh		;46b4	3e 0d 	> . 
	jp DOSError		; Error handling routine	;46b6	c3 41 42 	. A B 
l46b9h:
	push hl			;46b9	e5 	. 
l46bah:
	call READSECTOR		; Read a sector from disk						;46ba	cd 27 5b 	. ' [ 
	or a			;46bd	b7 	. 
	jp nz,DOSError		; Error handling routine	;46be	c2 41 42 	. A B 
	ld l,(iy+DataBuf)		;46c1	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)		;46c4	fd 66 32 	. f 2 
	...
	inc hl			;46cf	23 	# 
	ld (iy+UserTrackNo),a		;46d0	fd 77 12 	. w . 
	ld a,(hl)			;46d3	7e 	~ 
	ld (iy+UserSectorNo),a		;46d4	fd 77 11 	. w . 
	jr l46bah		;46d7	18 e1 	. . 
l46d9h:
	ld b,07eh		;46d9	06 7e 	. ~ 
	ld l,(iy+DataBuf)		;46db	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)		;46de	fd 66 32 	. f 2 
l46e1h:
	ld a,(hl)			;46e1	7e 	~ 
	inc hl			;46e2	23 	# 
	or a			;46e3	b7 	. 
	...
	call ReadMap		; Read the track map of the disk				;46e8	cd 17 47 	. . G 
	call FINDEMPTYSEC		; Search for empty sector						;46eb	cd bf 58 	. . X 
	or a			;46ee	b7 	. 
	jp nz,DOSError		; Error handling routine	;46ef	c2 41 42 	. A B 
	ld a,(iy+NextTrackNo)		;46f2	fd 7e 16 	. ~ . 
	ld (iy+UserTrackNo),a		;46f5	fd 77 12 	. w . 
	ld a,(iy+NextSectorNo)		;46f8	fd 7e 15 	. ~ . 
	ld (iy+UserSectorNo),a		;46fb	fd 77 11 	. w . 
	call ClearSector		; Clear a sector of the disk					;46fe	cd 49 47 	. I G 
...
l4703h:
	...
	ld (hl),a			;4707	77 	w 
	dec hl			;4708	2b 	+ 
	ld a,(iy+UserSectorNo)		;4709	fd 7e 11 	. ~ . 
	ld (hl),a			;470c	77 	w 
	dec hl			;470d	2b 	+ 
	ld a,(iy+UserTrackNo)		;470e	fd 7e 12 	. ~ . 
	ld (hl),a			;4711	77 	w 
	call PowerOff		; Disk power OFF		;4712	cd 52 5f 	. R _ 
	pop hl			;4715	e1 	. 
	ret			;4716	c9 	. 

;***************************************************************************************************
; Read the track map of the disk and place it into address pointed by iy+MapAddress
; IN: Disabled interrupts
; OUT: a - error code or 0 if no error
;***************************************************************************************************
ReadMap:
; -- read sector 15 on track 0
	ld (iy+UserTrackNo),0			; Track number 0												;4717	fd 36 12 00 	. 6 . . 
	ld (iy+UserSectorNo),15			; Sector Number 15												;471b	fd 36 11 0f 	. 6 . . 
	call READSECTOR					; Read a sector from disk into Sector Buffer					;471f	cd 27 5b 	. ' [ 
	or a							; check if error (a != 0)										;4722	b7 	. 
	jp nz,DOSError					; yes - jump to Error handling routine							;4723	c2 41 42 	. A B 
; -- copy map data from read buffer into MapData
	ld e,(iy+MapAddress)																			;4726	fd 5e 34 	. ^ 4 
	ld d,(iy+MapAddress+1)			; dst - de - MapAddress 										;4729	fd 56 35 	. V 5 
	ld l,(iy+DataBuf)																				;472c	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)				; src - hl - DataBuff											;472f	fd 66 32 	. f 2 
...
;***************************************************************************************************
; Fill DataBuf with value 0
; IN: none
; OUT: none
;***************************************************************************************************
ClearDataBuf:
	ld l,(iy+DataBuf)																				;4738	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)				; src - hl - DataBuf address									;473b	fd 66 32 	. f 2 
	ld (hl),0						; set first byte to 0											;473e	36 00 	6 . 
	ld e,l																							;4740	5d 	] 
	ld d,h							; dst - de - DataBuf address									;4741	54 	T 
...
;***************************************************************************************************
ClearSector:
	call ClearDataBuf				; clear 128 bytes in DataBuf									;4749	cd 38 47 	. 8 G 
	call WRITESECTOR				; Write a sector to disk										;474c	cd a1 59 	. . Y 
	or a							; check if error (a != 0)										;474f	b7 	. 
	jp nz,DOSError					; yes - jump to Error handling routine							;4750	c2 41 42 	. A B 
	ret								; -------------------------- End of Proc ----------------------	;4753	c9 	. 



;***************************************************************************************************
; Save the track map pointed by iy+MapAddress to the disk (track 0, sector 15)
; IN: Disabled interrupts
;     IY+DRVS - drive selected  
; OUT: a - error code or 0 if no error
;***************************************************************************************************
SaveMap:
; -- clear sector buffer
	call ClearDataBuf				; clear 128 bytes in DataBuf									;4754	cd 38 47 	. 8 G 

; -- setup parameters (track=0,sector=15)
	ld (iy+UserTrackNo),0			; Track number 0												;4757	fd 36 12 00 	. 6 . . 
	ld (iy+UserSectorNo),15			; Sector number 15												;475b	fd 36 11 0f 	. 6 . . 

; -- copy Sector Map data into Sector Data Buffer
	ld e,(iy+DataBuf)																				;475f	fd 5e 31 	. ^ 1 
	ld d,(iy+DataBuf+1)				; dst - de - address of DataBuf									;4762	fd 56 32 	. V 2 
	ld l,(iy+MapAddress)																			;4765	fd 6e 34 	. n 4 
	ld h,(iy+MapAddress+1)			; src - hl - address of MapData									;4768	fd 66 35 	. f 5 
	ld bc,80						; 80 bytes to copy (rest of sector is already cleared to 0)		;476b	01 50 00 	. P . 
	ldir							; copy bytes do DataBuf											;476e	ed b0 	. . 
; -- write buffer to Disk Sector
	call WRITESECTOR				; Write a sector to disk										;4770	cd a1 59 	. . Y 
	or a							; check if error (a != 0)										;4773	b7 	. 
	jp nz,DOSError					; yes - jump to Error handling routine							;4774	c2 41 42 	. A B 
...
; -- check if file is open and use FCB Block 1
	ld (iy+FileNo),0				; set File # to 0 - current used FCB Block 1					;477b	fd 36 00 00 	. 6 . . 
	ld de,FCBlock1					; de - offset to 1st FCB Block									;477f	11 17 00 	. . . 
...
; -- check if FCB Block 1 handles this file
	call FCBHandlesFile				; check if filenames in FCB and (iy+filename) match				;4787	cd bf 47 	. . G 
	cp 8							; is Error 8 - FILE ALREADY OPEN ?								;478a	fe 08 	. . 
	ret z							; yes - ------------------ End Of Proc ------------------------	;478c	c8 	. 

; -- 
	inc (iy+FileNo)					; set File # to 1 - current used FCB Block 2					;478d	fd 34 00 	. 4 . 
.checkFCB2:
...
; -- file is not Open 
	...
	ld de,FCBlock1					; de - offset to 1st FCB Block									;479b	11 17 00 	. . . 
	add hl,de						; hl - address of FCB Block	1									;479e	19 	. 
	ld a,(iy+FileNo)				; a - ;479f	fd 7e 00 	. ~ . 
	or a			;47a2	b7 	. 
	jr z,l47a9h		;47a3	28 04 	( . 
	ld de,0000dh		;47a5	11 0d 00 	. . . 
	add hl,de			;47a8	19 	. 
l47a9h:
...
l47ach:
	call FCBHandlesFile				; check if filenames in FCB and (iy+filename) match				;47ac	cd bf 47 	. . G 
	cp 8							; is Error 8 - FILE ALREADY OPEN ?								;47af	fe 08 	. . 
	ret z							; yes - ------------------ End Of Proc ------------------------	;47b1	c8 	. 
; -- any other errors
	or a							; clear Carry flag												;47b2	b7 	. 
	sbc hl,de						;47b3	ed 52 	. R 
	ld a,(iy+FCBlock1)		;47b5	fd 7e 17 	. ~ . 
	or a			;47b8	b7 	. 
...
;***************************************************************************************************
; Check if FCB specified by hl handles filename specified in iy+Filename  
...
.nextChar:
	ld a,(iy+Filename)				; a - char of filename to compare								;47c6	fd 7e 01 	. ~ . 
...
; -- parse required filename
	call CSI						; parse filename and copy it to (iy+filename)					;47dd	cd 67 53 	. g S 
	or a							; was any Error?												;47e0	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;47e1	c2 41 42 	. A B 
...
; -- called from direct command 
	call sub_4778h		;47ed	cd 78 47 	. x G 
	pop hl			;47f0	e1 	. 
	cp 008h		;47f1	fe 08 	. . 
	ret nz			;47f3	c0 	. 
	xor a			;47f4	af 	. 
...
.fromBasicProg:
	call sub_4778h		;47f7	cd 78 47 	. x G 
	pop hl			;47fa	e1 	. 
	cp 008h		;47fb	fe 08 	. . 
	ret nz			;47fd	c0 	. 
	ld a,(de)			;47fe	1a 	. 
	...
	ld a,(hl)			;480f	7e 	~ 
	inc hl			;4810	23 	# 
	ld (iy+UserTrackNo),a		;4811	fd 77 12 	. w . 
	ld a,(hl)			;4814	7e 	~ 
	ld (iy+UserSectorNo),a		;4815	fd 77 11 	. w . 
	di			;4818	f3 	. 
	call PowerOn		; Disk power ON			;4819	cd 41 5f 	. A _ 
	push bc			;481c	c5 	. 
	ld bc,00032h		;481d	01 32 00 	. 2 . 
	call DELAYMS		; Delay mS in reg C								;4820	cd be 5e 	. . ^ 
	pop bc			;4823	c1 	. 
	in a,(FLWR_PROTECT)		;4824	db 13 	. . 
	or a			;4826	b7 	. 
	ld a,004h		;4827	3e 04 	> . 
	jp m,DOSError		; Error handling routine	;4829	fa 41 42 	. A B 
	call WRITESECTOR		; Write a sector to disk						;482c	cd a1 59 	. . Y 
	or a			;482f	b7 	. 
	jp nz,DOSError		; Error handling routine	;4830	c2 41 42 	. A B 
	ei			;4833	fb 	. 
	call PowerOff		; Disk power OFF		;4834	cd 52 5f 	. R _ 
	pop hl			;4837	e1 	. 
	ret			;4838	c9 	. 
...
; -- expected required name of file enclosed with double quote chars
	...
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;483e	c2 41 42 	. A B 
...
; -- convert hex text (4 chars) into 16bit address - start of memory to save
	call Asc2Hex					; de - 16bit address from 4 chars of Hex text					;484e	cd b9 53 	. . S 
...
; -- convert hex text (4 chars) into 16bit address - end of memory to save
	call Asc2Hex					; de - 16bit address from 4 chars of Hex text					;485c	cd b9 53 	. . S 
...
; -- set type of file as Binary Program ('B')
	ld (iy+FileType),'B'			; set type of file as Binary Program ('B')						;4874	fd 36 09 42 	. 6 . B 
...
; -- verify start addres is smaller than end address 
	...
	jp nc,SF_ExitError				; if hl >= de - exit with Error 01 ----------------------------	;4881	d2 9a 48 	. . H 

; -- no error - turn on Disk Drive and wait 2ms
	call PowerOn					; Disk power ON													;4884	cd 41 5f 	. A _ 
	push bc							; save bc														;4887	c5 	. 
	ld bc,2							; bc - 2 miliseconds to delay									;4888	01 02 00 	. . . 
	call DELAYMS					; delay 2 ms 													;488b	cd be 5e 	. . ^ 
	pop bc							; restore bc													;488e	c1 	. 
; -- check if Disk is not Write-Protected
	in a,(FLWR_PROTECT)				; a - read Write Protected flag from FDC						;488f	db 13 	. . 
	or a							; is bit 7 set? (write protected)								;4891	b7 	. 
	ld a,4							; a - Error 04 - DISK WRITE PROTECTED							;4892	3e 04 	> . 
	jp m,SF_ExitError				; yes - go to Error handling routine --------------------------	;4894	fa 9a 48 	. . H 
...
; -- 
	call FINDFILE					; Search for file in directory									;489e	cd 13 59 	. . Y 
...
; -- there is Directory Entry for this file - mark as deleted 
	...
	call WRITESECTOR				; Write sector with Directory to Disk							;48ae	cd a1 59 	. . Y 
...
BS_ExitError:
	...
	jp DOSError						; got Error handling routine ----------------------------------	;48c1	c3 41 42 	. A B 
...
DCmdBLOAD:
; -- expected required name of file enclosed with double quote chars
	call CSI						; parse filename and copy it to (iy+filename)					;48c4	cd 67 53 	. g S 
	push hl							; save hl - parse point											;48c7	e5 	. 
	or a							; was any Error?												;48c8	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;48c9	c2 41 42 	. A B 

; -- set type of file as Binary Program ('B') 
	ld (iy+FileType),'B'			; set type of file as Binary Program ('B')						;48cc	fd 36 09 42 	. 6 . B 
...
; -- call DOS routine
	call LOADFILE					; Load a file from disk											;48dc	cd b1 43 	. . C 
...
; -- now check if was any Error 
	...
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;48eb	c2 41 42 	. A B 
...
DCmdBRUN:
; -- expected required name of file enclosed with double quote chars
	call CSI						; parse filename and copy it to (iy+filename)					;48ef	cd 67 53 	. g S 
	push hl							; save hl - parse point											;48f2	e5 	. 
	or a							; was any Error?												;48f3	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;48f4	c2 41 42 	. A B 

; -- set type of file as Binary Program ('B') and call DOS routine
	ld (iy+FileType),'B'			; set type of file as Binary Program ('B')						;48f7	fd 36 09 42 	. 6 . B 
	call LOADFILE					; Load a file from disk											;48fb	cd b1 43 	. . C 
	or a							; was any Error?												;48fe	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	; Error handling routine	;48ff	c2 41 42 	. A B 
...
; -- turn on Disk Drive and wait 50 ms
	call PowerOn					; Disk power ON													;4908	cd 41 5f 	. A _ 
	push bc							; save bc														;490b	c5 	. 
	ld bc,50						; bc - 50 miliseconds to delay									;490c	01 32 00 	. 2 . 
	call DELAYMS					; delay 50 ms													;490f	cd be 5e 	. . ^ 
	pop bc							; restore bc													;4912	c1 	. 

; -- set 1st sector with Directory Entries to read
	ld (iy+UserTrackNo),0			; set Track Number 0											;4913	fd 36 12 00 	. 6 . . 
	ld (iy+UserSectorNo),0			; set Sector Number 0											;4917	fd 36 11 00 	. 6 . . 

.nextSector:
; -- read Sector with Directory Entries 
	di								; disable interrupts											;491b	f3 	. 
	call READSECTOR					; Read sector from disk											;491c	cd 27 5b 	. ' [ 
	or a							; was any Error?												;491f	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;4920	c2 41 42 	. A B 

; -- setup for 1st Directory Entry on this Sector
	ld l,(iy+DataBuf)				; hl - address of data from Sector								;4923	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)																				;4926	fd 66 32 	. f 2 
...
; -- SPACE key is pressed - delay 20 ms 
	...
	call DELAYMS					; delay 20 ms													;4958	cd be 5e 	. . ^ 
...
; -- SPACE key is released - delay 20 ms 
	...
	call DELAYMS					; delay 20 ms													;4967	cd be 5e 	. . ^ 
...
; -- SPACE key is pressed - delay 20 ms 
	...
	call DELAYMS					; delay 20 ms													;4976	cd be 5e 	. . ^ 
...
; -- 8 entries been read - setup next Sector to read
	inc (iy+UserSectorNo)			; increment Sector Number										;4985	fd 34 11 	. 4 . 
	ld a,(iy+UserSectorNo)			; a - number of next Sector to read								;4988	fd 7e 11 	. ~ . 
	cp 15							; all 14 sectors already read?									;498b	fe 0f 	. . 
	jr nz,.nextSector				; no - read next Sector ---------------------------------------	;498d	20 8c 	  . 

.exit:
	call PowerOff					; Disk power OFF												;498f	cd 52 5f 	. R _ 
...
DCmdERA:
; -- expected required name of file enclosed with double quote chars
	call CSI						; parse filename and copy it to (iy+filename)					;4994	cd 67 53 	. g S 
	push hl							; save hl - parse point											;4997	e5 	. 
	or a							; was any Error?												;4998	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;4999	c2 41 42 	. A B 

; -- turn on Disk Drive and wait 50 ms
	di								; disable interrupts											;499c	f3 	. 
	call PowerOn					; Disk power ON													;499d	cd 41 5f 	. A _ 
	push bc							; save bc														;49a0	c5 	. 
	ld bc,50						; bc - 50 miliseconds to delay									;49a1	01 32 00 	. 2 . 
	call DELAYMS					; delay 50 ms													;49a4	cd be 5e 	. . ^ 
	pop bc							; restore bc													;49a7	c1 	. 

; -- check if Disk is not Write-Protected
	in a,(FLWR_PROTECT)				; a - read Write Protected flag from FDC						;49a8	db 13 	. . 
	or a							; is bit 7 set? (write protected)								;49aa	b7 	. 
	ld a,4							; a - Error 04 - DISK WRITE PROTECTED							;49ab	3e 04 	> . 
	jp m,DOSError					; yes - go to Error handling routine --------------------------	;49ad	fa 41 42 	. A B 

; -- find Directory Entry for this file
	call FINDFILE					; Search for file in directory									;49b0	cd 13 59 	. . Y 
	cp 2							; was Error 2 - FILE ALREADY EXISTS	?							;49b3	fe 02 	. . 
	jr z,.fileFound					; yes - continue deleting file --------------------------------	;49b5	28 09 	( . 
; -- other error or No Error
	or a							; other error?													;49b7	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;49b8	c2 41 42 	. A B 
; -- No Error - means there are no requested file on Disk
	ld a,13							; a - Error 13 - FILE NOT FOUND									;49bb	3e 0d 	> . 
	jp DOSError						; go to Error handling routine --------------------------------	;49bd	c3 41 42 	. A B 
...
; -- set track and sector numbers for 1st sector of file 
	...
	ld (iy+NextTrackNo),a			; set Track Number for Sector to delete 						;49c2	fd 77 16 	. w . 
	ld a,(de)						; a - Sector Number of 1st file sector							;49c5	1a 	. 
	ld (iy+NextSectorNo),a			; set Sector Number fo Sector to delete							;49c6	fd 77 15 	. w . 
...
; -- mark file's Directory Entry as deleted
	...
	call WRITESECTOR				; Write sector with Directory to disk							;49d0	cd a1 59 	. . Y 
	or a							; was any Error?												;49d3	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;49d4	c2 41 42 	. A B 

; -- read Disk Map Sector from Disk
	ld (iy+UserTrackNo),0			; set Track Number 0 											;49d7	fd 36 12 00 	. 6 . . 
	ld (iy+UserSectorNo),15			; set Sector Number 15											;49db	fd 36 11 0f 	. 6 . . 
	call READSECTOR					; Read Disk Map sector from disk								;49df	cd 27 5b 	. ' [ 
	or a							; was any Error?												;49e2	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;49e3	c2 41 42 	. A B 
; -- copy sector data to Disk Map buffer
	ld e,(iy+MapAddress)			; de - (dst) address of Disk Map buffer							;49e6	fd 5e 34 	. ^ 4 
	ld d,(iy+MapAddress+1)																			;49e9	fd 56 35 	. V 5 
	ld l,(iy+DataBuf)				; hl - (src) address of Sector data read from Disk				;49ec	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)																				;49ef	fd 66 32 	. f 2 
...
.nextFileSector:
; -- read file sector 
	ld a,(iy+NextTrackNo)			; a - Track  Number to read										;49f7	fd 7e 16 	. ~ . 
	or a							; is it 0? (no more sectors used by file)						;49fa	b7 	. 
	jp z,.saveDiskMap				; yes - ;49fb	ca 4f 4a 	. O J 
	ld (iy+UserTrackNo),a			; set as Track Number of sector to read							;49fe	fd 77 12 	. w . 
	ld a,(iy+NextSectorNo)			; a - Sector Number												;4a01	fd 7e 15 	. ~ . 
	ld (iy+UserSectorNo),a			; set as Sector Number of sector to read						;4a04	fd 77 11 	. w . 
	call READSECTOR					; Read a sector from disk										;4a07	cd 27 5b 	. ' [ 
	or a							; was any Error?												;4a0a	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;4a0b	c2 41 42 	. A B 
; -- read last 2 bytes to get next sector number and track number
	ld l,(iy+DataBuf)				; hl - address of Sector data 									;4a0e	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)																				;4a11	fd 66 32 	. f 2 
	...
	ld (iy+NextTrackNo),a			; store as next Track Number									;4a19	fd 77 16 	. w . 
	inc hl							; hl - points to next Sector Number								;4a1c	23 	# 
	ld a,(hl)						; a - sector number of next sector								;4a1d	7e 	~ 
	ld (iy+NextSectorNo),a			; stor as next Sector Number									;4a1e	fd 77 15 	. w . 

; -- get Disk Map address 
	ld l,(iy+MapAddress)			; hl - address of Disk Map buffer								;4a21	fd 6e 34 	. n 4 
	ld h,(iy+MapAddress+1)																			;4a24	fd 66 35 	. f 5 
; -- calculate byte offset 
	ld a,(iy+UserTrackNo)			; a - Track Number of sector for next chunk of file				;4a27	fd 7e 12 	. ~ . 
	...
	ld a,(iy+UserSectorNo)			; a - Sector number for next chunk of file						;4a30	fd 7e 11 	. ~ . 
...
.saveDiskMap:
; -- clear Sector Buffer
	ld l,(iy+DataBuf)				; hl - (src) address of Sector Buffer							;4a4f	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)																				;4a52	fd 66 32 	. f 2 
...
; -- copy 80 bytes of Disk Map to Sector Buffer	
	...
	ld l,(iy+MapAddress)			; hl - (src) address of Disk Map buffer							;4a61	fd 6e 34 	. n 4 
	ld h,(iy+MapAddress+1)																			;4a64	fd 66 35 	. f 5 
	ld bc,80						; bc - 80 bytes to copy											;4a67	01 50 00 	. P . 
	ldir							; copy Disk Map to Sector Buffer								;4a6a	ed b0 	. . 
; -- save Disk Map to Disk
	ld (iy+UserTrackNo),0			; set Track Number 0											;4a6c	fd 36 12 00 	. 6 . . 
	ld (iy+UserSectorNo),15			; set Sector Number 15											;4a70	fd 36 11 0f 	. 6 . . 
	call WRITESECTOR				; Write Disk Map to sector on Disk								;4a74	cd a1 59 	. . Y 
	or a							; was any Error?												;4a77	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;4a78	c2 41 42 	. A B 
; -- file deleted 
	call PowerOff					; Disk power OFF												;4a7b	cd 52 5f 	. R _ 
...
; -- expected required name of old file enclosed with double quote chars
	...
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;4a85	c2 41 42 	. A B 
; -- expected ',' char 
...
; -- expected required name of new file enclosed with double quote chars
	call CSI						; parse new filename and copy it to (iy+filename)				;4a8a	cd 67 53 	. g S 
	or a							; was any Error?												;4a8d	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;4a8e	c2 41 42 	. A B 
; -- 
...
; -- turn on Disk Drive and wait 50 ms
	...
	call PowerOn					; Disk power ON													;4a99	cd 41 5f 	. A _ 
	push bc							; save bc														;4a9c	c5 	. 
	ld bc,50						; bc - 50 miliseconds to delay									;4a9d	01 32 00 	. 2 . 
	call DELAYMS					; delay 50 ms													;4aa0	cd be 5e 	. . ^ 
	pop bc							; restore bc 													;4aa3	c1 	. 

; -- check if Disk is not Write-Protected
	in a,(FLWR_PROTECT)				; a - read Write Protected flag from FDC						;4aa4	db 13 	. . 
	or a							; is bit 7 set? (write protected)								;4aa6	b7 	. 
	ld a,4							; a - Error 04 - DISK WRITE PROTECTED							;4aa7	3e 04 	> . 
	jp m,DOSError					; yes - go to Error handling routine --------------------------	;4aa9	fa 41 42 	. A B 

; -- find Directory Entry for this file
	call FINDFILE					; Search for old filename in directory							;4aac	cd 13 59 	. . Y 
	cp 2							; was Error 2 - FILE ALREADY EXISTS	?							;4aaf	fe 02 	. . 
	jp z,.oldFilenameFound			; yes - continue renaming file --------------------------------	;4ab1	ca bd 4a 	. . J 

; -- other error or No Error
	or a							; other error?													;4ab4	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;4ab5	c2 41 42 	. A B 

; -- No Error - means there are no requested file on Disk
	ld a,13							; a - Error 13 - FILE NOT FOUND									;4ab8	3e 0d 	> . 
	jp DOSError						; go to Error handling routine --------------------------------	;4aba	c3 41 42 	. A B 

.oldFilenameFound:
	pop hl							; restore hl - start of new filename text						;4abd	e1 	. 
	call CSI						; parse new filename and copy it to (iy+filename)				;4abe	cd 67 53 	. g S 
...
; -- check if already exists file with new filename in Directory Entry
	call FINDFILE					; Search for new filename in directory							;4ac3	cd 13 59 	. . Y 
...
; -- other error or No Error
	...
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;4acb	c2 41 42 	. A B 
...
; -- get sector wit directory Entry for file to rename
	...
	call FINDFILE					; find Directory Entry for old filename							;4ad4	cd 13 59 	. . Y 
	cp 2							; was Error 2 - FILE ALREADY EXISTS ?							;4ad7	fe 02 	. . 
	jp nz,DOSError					; no - go to Error handling routine ---------------------------	;4ad9	c2 41 42 	. A B 

; -- save hl and de registers (end of filename in DOS buffer and Directory Entry buffer)
	pop bc							; bc - start of new filename									;4adc	c1 	. 
	push hl							; save hl - end of old filename in (iy+filename)				;4add	e5 	. 
...
; -- copy given new filename to DOS buffer
	...
	call CSI						; parse new filename and copy it to (iy+filename)				;4ae1	cd 67 53 	. g S 
; -- restore hl and de pointers and move them to begin of filename in DOS buffer and Dir Entry
	...
	pop hl							; restore hl - end of now new filename in (iy+filename)			;4ae5	e1 	. 
...
; -- copy FileType
	ld a,(iy+FileType+1)			; a - new file Type												;4aee	fd 7e 0a 	. ~ . 
...
; -- copy 8 chars of filename
	...
	call WRITESECTOR				; Write sector with Directory to disk							;4afc	cd a1 59 	. . Y 
	or a							; was any Error?												;4aff	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine --------------------------	;4b00	c2 41 42 	. A B 
; -- no error - turn off DIsk and return
	call PowerOff					; Disk power OFF												;4b03	cd 52 5f 	. R _ 
...
; -- prepeare sector data to copy on disk 
	...
	ld (iy+UserTrackNo),0			; set Track Number 0											;4b24	fd 36 12 00 	. 6 . . 
	ld (iy+UserSectorNo),0			; set Sector Number 0											;4b28	fd 36 11 00 	. 6 . . 
	ld (iy+CopyBuf),l				; set Sector buffer as address to copy from						;4b2c	fd 75 0e 	. u . 
	ld (iy+CopyBuf+1),h																				;4b2f	fd 74 0f 	. t . 
...
; -- move drive Head to track 0
	ld (iy+StepPhase),%00010001		; set Step Motor to 0001-0001 									;4b45	fd 36 38 11 	. 6 8 . 
	ld b,40							; max 40 tracks to StepOut										;4b49	06 28 	. ( 
	call STEPOUT					; execute Track Step Out 										;4b4b	cd 01 5f 	. . _ 
; -- wait 400ms 
	push bc							; save bc														;4b4e	c5 	. 
	ld bc,400						; 400 ms delay													;4b4f	01 90 01 	. . . 
	call DELAYMS					; wait 400 ms Delay												;4b52	cd be 5e 	. . ^ 
	pop bc							; restore bc													;4b55	c1 	. 

; -- set registers hl,de,bc to adresses in buffer for Trk#,Sect#,TSCRC
	ld l,(iy+CopyBuf)				; hl - address of Copy buffer									;4b56	fd 6e 0e 	. n . 
	ld h,(iy+CopyBuf+1)																				;4b59	fd 66 0f 	. f . 
...
; -- wait 100 ms delay
	...
	call DELAYMS					; wait 100 ms Delay 											;4b6b	cd be 5e 	. . ^ 
	pop bc							; restore bc													;4b6e	c1 	. 

; -- send Write Request to drive
	ld a,(iy+FLCtrlShadow)			; a - value last sent to FLCtrl									;4b6f	fd 7e 33 	. ~ 3 
	res 6,a							; clear bit 6 - Write Request (active LOW)						;4b72	cb b7 	. . 
	ld (iy+FLCtrlShadow),a			; store FLCtrl to shadow register								;4b74	fd 77 33 	. w 3 
	out (FLCTRL),a					; set Flopy Control byte										;4b77	d3 10 	. . 

; -- wait 100 ms delay
	push bc							; save bc 														;4b79	c5 	. 
	ld bc,100						; 100 ms Delay													;4b7a	01 64 00 	. d . 
	call DELAYMS					; wait 100 ms Delay 											;4b7d	cd be 5e 	. . ^ 
	pop bc							; restore bc													;4b80	c1 	. 
; -- 
	ld ix,SectorsSequence			; ix - table of Sectors on Track interlave sequence				;4b81	dd 21 67 4d 	. ! g M 

DI_WriteSector:
	ld l,(iy+CopyBuf)				; hl - address of Copy buffer 									;4b85	fd 6e 0e 	. n . 
	ld h,(iy+CopyBuf+1)																				;4b88	fd 66 0f 	. f . 
	ld d,(iy+FLCtrlShadow)			; d - value last sent to FLCtrl									;4b8b	fd 56 33 	. V 3 
...
; -- update FLCtrl shadow
	ld (iy+FLCtrlShadow),d			; store value last sent to FLCtrl								;4cc7	fd 72 33 	. r 3 
...
; -- turn off Write Request
	ld a,(iy+FLCtrlShadow)			; a - FLCtrl value from shadow register							;4ce7	fd 7e 33 	. ~ 3 
	or FL_WRITE_REQ					; turn off Write Request										;4cea	f6 40 	. @ 
	ld (iy+FLCtrlShadow),a			; store FLCtrl to shadow register								;4cec	fd 77 33 	. w 3 
	out (FLCTRL),a					; set Flopy Control byte										;4cef	d3 10 	. . 

; -- advance to next track
	ld b,1							; 1 track to step in											;4cf1	06 01 	. . 
	call STEPIN						; move Read/Write Head in drive									;4cf3	cd ce 5e 	. . ^ 
	jp DI_WriteTrack				; write next track												;4cf6	c3 67 4b 	. g K 


DI_VerifyDisk:
; -- turn off Write Request
	ld a,(iy+FLCtrlShadow)			; a - FLCtrl value												;4cf9	fd 7e 33 	. ~ 3 
	or FL_WRITE_REQ					; turn off Write Request										;4cfc	f6 40 	. @ 
	ld (iy+FLCtrlShadow),a			; store FLCtrl to shadow register								;4cfe	fd 77 33 	. w 3 
...
; -- move drive Head to track 0, sector 0
	...
	call STEPOUT					; move Read/Write Head in drive									;4d05	cd 01 5f 	. . _ 
	ld (iy+UserTrackNo),0			; set Track Number to 0											;4d08	fd 36 12 00 	. 6 . . 
	ld (iy+UserSectorNo),0			; set Sector Number to 0										;4d0c	fd 36 11 00 	. 6 . . 
...
; -- try read IDAM for given Track and Sector (only IDAM start, TrackNo, SectorNo and verify Checksum of those two)
	call ReadIDAddressMark			; Read identification address mark (IDAM) 						;4d14	cd ea 53 	. . S 
...
; -- sector found
	...
	ld (iy+UserSectorNo),a			; set Sector Number	to next seq value							;4d1e	fd 77 11 	. w . 
...
; -- next track
	...
	ld (iy+UserSectorNo),a			; set Sector Number to 0										;4d26	fd 77 11 	. w . 
	ld a,(iy+UserTrackNo)			; a - current Track number										;4d29	fd 7e 12 	. ~ . 
	inc a							; increment track												;4d2c	3c 	< 
	ld (iy+UserTrackNo),a			; set next track number											;4d2d	fd 77 12 	. w . 
...
; -- more tracks to verify
	...
	call STEPIN						; move R/W Head to next track									;4d36	cd ce 5e 	. . ^ 
...
; -- move Head back on Track 00
	...
	call STEPOUT					; Track step out												;4d3d	cd 01 5f 	. . _ 
	call PowerOff					; Disk power OFF												;4d40	cd 52 5f 	. R _ 
	pop hl							; restore hl 													;4d43	e1 	. 
	ret								; ------------- End of Proc -----------------------------------	;4d44	c9 	. 
DI_ExitError:
	cp 17							; is it Error 17 (BREAK)										;4d45	fe 11 	. . 
	jp z,DOSError					; yes - jump to Error handling routine							;4d47	ca 41 42 	. A B 
	ld a,6							; no - set Error 6 (DISK I/O ERROR)								;4d4a	3e 06 	> . 
	jp DOSError						; jump to Error handling routine ------------------------------ ;4d4c	c3 41 42 	. A B 
...
DCmdDRIVE:
	call 02b1ch		;4d78	cd 1c 2b 	. . + 
	or a			;4d7b	b7 	. 
	jp z,01e4ah		;4d7c	ca 4a 1e 	. J . 
	cp 003h		;4d7f	fe 03 	. . 
	jp nc,01e4ah		;4d81	d2 4a 1e 	. J . 
sub_4d84h:
	cp 001h		;4d84	fe 01 	. . 
	jr nz,l4d8dh		;4d86	20 05 	  . 
	ld (iy+SelectedDrive),010h		;4d88	fd 36 0b 10 	. 6 . . 
	ret			;4d8c	c9 	. 
l4d8dh:
	ld (iy+SelectedDrive),080h		;4d8d	fd 36 0b 80 	. 6 . . 
	ret			;4d91	c9 	. 
...
; NOTE: This command must always be used from inside BASIC program. 
DCmdIN#:
	call 02828h		;4d92	cd 28 28 	. ( ( 
	call ParseFilename		;4d95	cd 78 53 	. x S 
	or a			;4d98	b7 	. 
	jp nz,DOSError		; Error handling routine	;4d99	c2 41 42 	. A B 
	rst 8			;4d9c	cf 	. 
	inc l			;4d9d	2c 	, 
	push hl			;4d9e	e5 	. 
	call sub_4778h		;4d9f	cd 78 47 	. x G 
	cp 008h		;4da2	fe 08 	. . 
	ld a,005h		;4da4	3e 05 	> . 
	jp nz,DOSError		; Error handling routine	;4da6	c2 41 42 	. A B 
	inc de			;4da9	13 	. 
	ld a,(de)			;4daa	1a 	. 
	or a			;4dab	b7 	. 
	ld a,00fh		;4dac	3e 0f 	> . 
	jp nz,DOSError		; Error handling routine	;4dae	c2 41 42 	. A B 
	dec de			;4db1	1b 	. 
	ld a,(de)			;4db2	1a 	. 
	cp 002h		;4db3	fe 02 	. . 
	...
	ld a,(hl)			;4dc2	7e 	~ 
	inc hl			;4dc3	23 	# 
	ld (iy+UserTrackNo),a		;4dc4	fd 77 12 	. w . 
	ld a,(hl)			;4dc7	7e 	~ 
	ld (iy+UserSectorNo),a		;4dc8	fd 77 11 	. w . 
	di			;4dcb	f3 	. 
	call PowerOn		; Disk power ON			;4dcc	cd 41 5f 	. A _ 
	push bc			;4dcf	c5 	. 
	ld bc,00032h		;4dd0	01 32 00 	. 2 . 
	call DELAYMS		; Delay mS in reg C								;4dd3	cd be 5e 	. . ^ 
	pop bc			;4dd6	c1 	. 
	call READSECTOR		; Read a sector from disk						;4dd7	cd 27 5b 	. ' [ 
	or a			;4dda	b7 	. 
	jp nz,DOSError		; Error handling routine	;4ddb	c2 41 42 	. A B 
	ei			;4dde	fb 	. 
	call PowerOff		; Disk power OFF		;4ddf	cd 52 5f 	. R _ 
l4de2h:
...
sub_4df9h:
	...
	push de			;4dfa	d5 	. 
	push bc			;4dfb	c5 	. 
	call sub_4778h		;4dfc	cd 78 47 	. x G 
	ld hl,0000ch		;4dff	21 0c 00 	! . . 
	ex de,hl			;4e02	eb 	. 
	add hl,de			;4e03	19 	. 
	ld a,(hl)			;4e04	7e 	~ 
	ex de,hl			;4e05	eb 	. 
	ld l,(iy+DataBuf)		;4e06	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)		;4e09	fd 66 32 	. f 2 
	add a,l			;4e0c	85 	. 
	ld l,a			;4e0d	6f 	o 
...
l4e1ah:
	...
	xor a			;4e22	af 	. 
	ld (de),a			;4e23	12 	. 
	ld l,(iy+DataBuf)		;4e24	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)		;4e27	fd 66 32 	. f 2 
	push de			;4e2a	d5 	. 
	...
	ld (iy+UserTrackNo),a		;4e34	fd 77 12 	. w . 
	dec de			;4e37	1b 	. 
	dec de			;4e38	1b 	. 
	ld (de),a			;4e39	12 	. 
	inc hl			;4e3a	23 	# 
	ld a,(hl)			;4e3b	7e 	~ 
	ld (iy+UserSectorNo),a		;4e3c	fd 77 11 	. w . 
	inc de			;4e3f	13 	. 
	ld (de),a			;4e40	12 	. 
	di			;4e41	f3 	. 
	call PowerOn		; Disk power ON			;4e42	cd 41 5f 	. A _ 
l4e45h:
	push bc			;4e45	c5 	. 
	ld bc,00032h		;4e46	01 32 00 	. 2 . 
	call DELAYMS		; Delay mS in reg C								;4e49	cd be 5e 	. . ^ 
	pop bc			;4e4c	c1 	. 
	push bc			;4e4d	c5 	. 
	call READSECTOR		; Read a sector from disk						;4e4e	cd 27 5b 	. ' [ 
	pop bc			;4e51	c1 	. 
	or a			;4e52	b7 	. 
	jp nz,DOSError		; Error handling routine	;4e53	c2 41 42 	. A B 
	call PowerOff		; Disk power OFF		;4e56	cd 52 5f 	. R _ 
	ei			;4e59	fb 	. 
l4e5ah:
	ld a,c			;4e5a	79 	y 
	pop bc			;4e5b	c1 	. 
...
; NOTE: This command must always be used from inside BASIC program. 
DCmdPR#:
	call 02828h		;4e64	cd 28 28 	. ( ( 
	call ParseFilename		;4e67	cd 78 53 	. x S 
	or a			;4e6a	b7 	. 
	jp nz,DOSError		; Error handling routine	;4e6b	c2 41 42 	. A B 
	push hl			;4e6e	e5 	. 
	call sub_4778h		;4e6f	cd 78 47 	. x G 
	cp 008h		;4e72	fe 08 	. . 
	ld a,005h		;4e74	3e 05 	> . 
	jp nz,DOSError		; Error handling routine	;4e76	c2 41 42 	. A B 
	pop hl			;4e79	e1 	. 
	rst 8			;4e7a	cf 	. 
	inc l			;4e7b	2c 	, 
l4e7ch:
...
sub_4ecah:
	...
	push bc			;4ecc	c5 	. 
	push af			;4ecd	f5 	. 
	call sub_4778h		;4ece	cd 78 47 	. x G 
	ex de,hl			;4ed1	eb 	. 
	inc hl			;4ed2	23 	# 
	ld a,(hl)			;4ed3	7e 	~ 
	or a			;4ed4	b7 	. 
	ld a,010h		;4ed5	3e 10 	> . 
	jp z,DOSError		; Error handling routine	;4ed7	ca 41 42 	. A B 
	dec hl			;4eda	2b 	+ 
	ld a,(hl)			;4edb	7e 	~ 
	cp 002h		;4edc	fe 02 	. . 
	...
	ld a,(hl)			;4ee7	7e 	~ 
	inc hl			;4ee8	23 	# 
	ld (iy+UserTrackNo),a		;4ee9	fd 77 12 	. w . 
	ld a,(hl)			;4eec	7e 	~ 
	inc hl			;4eed	23 	# 
	ld (iy+UserSectorNo),a		;4eee	fd 77 11 	. w . 
	di			;4ef1	f3 	. 
	call PowerOn		; Disk power ON			;4ef2	cd 41 5f 	. A _ 
	in a,(FLWR_PROTECT)		;4ef5	db 13 	. . 
	or a			;4ef7	b7 	. 
	ld a,004h		;4ef8	3e 04 	> . 
	jp m,DOSError		; Error handling routine	;4efa	fa 41 42 	. A B 
	push hl			;4efd	e5 	. 
	call READSECTOR		; Read a sector from disk						;4efe	cd 27 5b 	. ' [ 
	or a			;4f01	b7 	. 
	jp nz,DOSError		; Error handling routine	;4f02	c2 41 42 	. A B 
	pop hl			;4f05	e1 	. 
...
l4f0ch:
	...
	ld l,(iy+DataBuf)		;4f14	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)		;4f17	fd 66 32 	. f 2 
	add hl,de			;4f1a	19 	. 
	pop af			;4f1b	f1 	. 
	push af			;4f1c	f5 	. 
	ld (hl),a			;4f1d	77 	w 
	...
	di			;4f24	f3 	. 
	call PowerOn		; Disk power ON			;4f25	cd 41 5f 	. A _ 
	push bc			;4f28	c5 	. 
	ld bc,00002h		;4f29	01 02 00 	. . . 
	call DELAYMS		; Delay mS in reg C								;4f2c	cd be 5e 	. . ^ 
	pop bc			;4f2f	c1 	. 
	in a,(FLWR_PROTECT)		;4f30	db 13 	. . 
	or a			;4f32	b7 	. 
	ld a,004h		;4f33	3e 04 	> . 
	jp m,DOSError		; Error handling routine	;4f35	fa 41 42 	. A B 
	ld e,(iy+UserSectorNo)		;4f38	fd 5e 11 	. ^ . 
	ld d,(iy+UserTrackNo)		;4f3b	fd 56 12 	. V . 
	push de			;4f3e	d5 	. 
	call WRITESECTOR		; Write a sector to disk						;4f3f	cd a1 59 	. . Y 
	or a			;4f42	b7 	. 
sub_4f43h:
	jp nz,DOSError		; Error handling routine	;4f43	c2 41 42 	. A B 
	call ReadMap		; Read the track map of the disk				;4f46	cd 17 47 	. . G 
	or a			;4f49	b7 	. 
	jp nz,DOSError		; Error handling routine	;4f4a	c2 41 42 	. A B 
	call FINDEMPTYSEC		; Search for empty sector						;4f4d	cd bf 58 	. . X 
	or a			;4f50	b7 	. 
	jp nz,DOSError		; Error handling routine	;4f51	c2 41 42 	. A B 
	call SaveMap		; Save the track map to the disk				;4f54	cd 54 47 	. T G 
	pop de			;4f57	d1 	. 
	ld (iy+UserSectorNo),e		;4f58	fd 73 11 	. s . 
	ld (iy+UserTrackNo),d		;4f5b	fd 72 12 	. r . 
	call READSECTOR		; Read a sector from disk						;4f5e	cd 27 5b 	. ' [ 
	or a			;4f61	b7 	. 
	jp nz,DOSError		; Error handling routine	;4f62	c2 41 42 	. A B 
	ld l,(iy+DataBuf)		;4f65	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)		;4f68	fd 66 32 	. f 2 
	ld de,0007eh		;4f6b	11 7e 00 	. ~ . 
	add hl,de			;4f6e	19 	. 
	ld a,(iy+NextTrackNo)		;4f6f	fd 7e 16 	. ~ . 
	ld (hl),a			;4f72	77 	w 
	inc hl			;4f73	23 	# 
	ld a,(iy+NextSectorNo)		;4f74	fd 7e 15 	. ~ . 
	ld (hl),a			;4f77	77 	w 
	call WRITESECTOR		; Write a sector to disk						;4f78	cd a1 59 	. . Y 
	or a			;4f7b	b7 	. 
	jp nz,DOSError		; Error handling routine	;4f7c	c2 41 42 	. A B 
	call sub_4778h		;4f7f	cd 78 47 	. x G 
	ex de,hl			;4f82	eb 	. 
	ld de,0000ah		;4f83	11 0a 00 	. . . 
	add hl,de			;4f86	19 	. 
	ld a,(iy+NextTrackNo)		;4f87	fd 7e 16 	. ~ . 
	ld (iy+UserTrackNo),a		;4f8a	fd 77 12 	. w . 
	ld (hl),a			;4f8d	77 	w 
	inc hl			;4f8e	23 	# 
	ld a,(iy+NextSectorNo)		;4f8f	fd 7e 15 	. ~ . 
	ld (iy+UserSectorNo),a		;4f92	fd 77 11 	. w . 
	ld (hl),a			;4f95	77 	w 
	inc hl			;4f96	23 	# 
	xor a			;4f97	af 	. 
	ld (hl),a			;4f98	77 	w 
	call ClearSector		; Clear a sector of the disk					;4f99	cd 49 47 	. I G 
l4f9ch:
	call PowerOff		; Disk power OFF		;4f9c	cd 52 5f 	. R _ 
	ei			;4f9f	fb 	. 
	pop af			;4fa0	f1 	. 
	pop bc			;4fa1	c1 	. 
	pop de			;4fa2	d1 	. 
...
sub_4fbbh:
	...
	add hl,de			;4fcb	19 	. 
	ld a,(hl)			;4fcc	7e 	~ 
	ld (iy+UserTrackNo),a		;4fcd	fd 77 12 	. w . 
	inc hl			;4fd0	23 	# 
	ld a,(hl)			;4fd1	7e 	~ 
	ld (iy+UserSectorNo),a		;4fd2	fd 77 11 	. w . 
	di			;4fd5	f3 	. 
	call PowerOn		; Disk power ON			;4fd6	cd 41 5f 	. A _ 
	push bc			;4fd9	c5 	. 
	ld bc,00032h		;4fda	01 32 00 	. 2 . 
	call DELAYMS		; Delay mS in reg C								;4fdd	cd be 5e 	. . ^ 
	pop bc			;4fe0	c1 	. 
	in a,(FLWR_PROTECT)		;4fe1	db 13 	. . 
	or a			;4fe3	b7 	. 
	ld a,004h		;4fe4	3e 04 	> . 
	jp m,DOSError		; Error handling routine	;4fe6	fa 41 42 	. A B 
	push hl			;4fe9	e5 	. 
	call WRITESECTOR		; Write a sector to disk						;4fea	cd a1 59 	. . Y 
	pop hl			;4fed	e1 	. 
	or a			;4fee	b7 	. 
	jp nz,DOSError		; Error handling routine	;4fef	c2 41 42 	. A B 
	ld de,0fff5h		;4ff2	11 f5 ff 	. . . 
	add hl,de			;4ff5	19 	. 
	call PowerOff		; Disk power OFF		;4ff6	cd 52 5f 	. R _ 
	ei			;4ff9	fb 	. 
	ret			;4ffa	c9 	. 
...
DCmdDCOPY:
	...
	ld (iy+039h),001h		;5007	fd 36 39 01 	. 6 9 . 
	dec hl			;500b	2b 	+ 
	rst 10h			;500c	d7 	. 
	jr z,l505dh		;500d	28 4e 	( N 
	call CSI		; Command string interpreter					;500f	cd 67 53 	. g S 
	or a			;5012	b7 	. 
	jp nz,DOSError		; Error handling routine	;5013	c2 41 42 	. A B 
	push hl			;5016	e5 	. 
	call sub_5168h		;5017	cd 68 51 	. h Q 
	call sub_5219h		;501a	cd 19 52 	. . R 
	di			;501d	f3 	. 
	call PowerOn		; Disk power ON			;501e	cd 41 5f 	. A _ 
	push bc			;5021	c5 	. 
	ld bc,00032h		;5022	01 32 00 	. 2 . 
	call DELAYMS		; Delay mS in reg C								;5025	cd be 5e 	. . ^ 
	pop bc			;5028	c1 	. 
	call FINDFILE		; Search for file in directory					;5029	cd 13 59 	. . Y 
	cp 002h		;502c	fe 02 	. . 
	jr z,l5039h		;502e	28 09 	( . 
	or a			;5030	b7 	. 
	jp nz,DOSError		; Error handling routine	;5031	c2 41 42 	. A B 
	ld a,00dh		;5034	3e 0d 	> . 
	jp DOSError		; Error handling routine	;5036	c3 41 42 	. A B 
l5039h:
	ld a,(iy+FileType+1)		;5039	fd 7e 0a 	. ~ . 
	ld (iy+FileType),a		;503c	fd 77 09 	. w . 
	cp 044h		;503f	fe 44 	. D 
	ld a,00ch		;5041	3e 0c 	> . 
	jp z,DOSError		; Error handling routine	;5043	ca 41 42 	. A B 
	call sub_43d3h		;5046	cd d3 43 	. . C 
	or a			;5049	b7 	. 
	jp nz,l5162h		;504a	c2 62 51 	. b Q 
	call sub_5275h		;504d	cd 75 52 	. u R 
	call PowerOn		; Disk power ON			;5050	cd 41 5f 	. A _ 
	call SAVEFILE		; Save a file to disk							;5053	cd 6e 44 	. n D 
	or a			;5056	b7 	. 
...
l505dh:
	...
	srl h		;506a	cb 3c 	. < 
	srl h		;506c	cb 3c 	. < 
	ld (iy+CopyTrackCount),h		;506e	fd 74 36 	. t 6 
	ld (iy+CopyTrackPtr),000h		;5071	fd 36 37 00 	. 6 7 . 
	ld (iy+UserTrackNo),000h		;5075	fd 36 12 00 	. 6 . . 
	ld (iy+UserSectorNo),000h		;5079	fd 36 11 00 	. 6 . . 
...
l5083h:
	...
	di			;508d	f3 	. 
	call PowerOn		; Disk power ON			;508e	cd 41 5f 	. A _ 
l5091h:
	call READSECTOR		; Read a sector from disk						;5091	cd 27 5b 	. ' [ 
	or a			;5094	b7 	. 
	jp nz,l5162h		;5095	c2 62 51 	. b Q 
	ld l,(iy+DataBuf)		;5098	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)		;509b	fd 66 32 	. f 2 
	...
	inc (iy+UserSectorNo)		;50ab	fd 34 11 	. 4 . 
	ld a,(iy+UserSectorNo)		;50ae	fd 7e 11 	. ~ . 
	cp 010h		;50b1	fe 10 	. . 
	jr nz,l5091h		;50b3	20 dc 	  . 
	ld (iy+UserSectorNo),000h		;50b5	fd 36 11 00 	. 6 . . 
	inc (iy+UserTrackNo)		;50b9	fd 34 12 	. 4 . 
	ld a,(iy+UserTrackNo)		;50bc	fd 7e 12 	. ~ . 
	cp 028h		;50bf	fe 28 	. ( 
	jr z,l50cbh		;50c1	28 08 	( . 
	sub (iy+CopyTrackPtr)		;50c3	fd 96 37 	. . 7 
	sub (iy+CopyTrackCount)		;50c6	fd 96 36 	. . 6 
	jr nz,l5091h		;50c9	20 c6 	  . 
l50cbh:
	ld a,(iy+CopyTrackPtr)		;50cb	fd 7e 37 	. ~ 7 
	ld (iy+UserTrackNo),a		;50ce	fd 77 12 	. w . 
	call PowerOff		; Disk power OFF		;50d1	cd 52 5f 	. R _ 
	call sub_5275h		;50d4	cd 75 52 	. u R 
	di			;50d7	f3 	. 
	call PowerOn		; Disk power ON			;50d8	cd 41 5f 	. A _ 
	push bc			;50db	c5 	. 
	ld bc,00002h		;50dc	01 02 00 	. . . 
	call DELAYMS		; Delay mS in reg C								;50df	cd be 5e 	. . ^ 
	pop bc			;50e2	c1 	. 
	in a,(FLWR_PROTECT)		;50e3	db 13 	. . 
	or a			;50e5	b7 	. 
	ld a,004h		;50e6	3e 04 	> . 
	jp m,l5162h		;50e8	fa 62 51 	. b Q 
...
l50f1h:
	...
	ld e,(iy+DataBuf)		;50f4	fd 5e 31 	. ^ 1 
	ld d,(iy+DataBuf+1)		;50f7	fd 56 32 	. V 2 
	ld bc,00080h		;50fa	01 80 00 	. . . 
	ldir		;50fd	ed b0 	. . 
	ld (SYS_BASIC_START_PTR),hl		; start of current BASIC Program ;50ff	22 a4 78 	" . x 
	call WRITESECTOR		; Write a sector to disk						;5102	cd a1 59 	. . Y 
	or a			;5105	b7 	. 
	jr nz,l5162h		;5106	20 5a 	  Z 
	inc (iy+UserSectorNo)		;5108	fd 34 11 	. 4 . 
	ld a,(iy+UserSectorNo)		;510b	fd 7e 11 	. ~ . 
	cp 010h		;510e	fe 10 	. . 
	jr nz,l50f1h		;5110	20 df 	  . 
	ld (iy+UserSectorNo),000h		;5112	fd 36 11 00 	. 6 . . 
	inc (iy+UserTrackNo)		;5116	fd 34 12 	. 4 . 
	ld a,(iy+UserTrackNo)		;5119	fd 7e 12 	. ~ . 
	cp 028h		;511c	fe 28 	. ( 
	jr z,l5137h		;511e	28 17 	( . 
	sub (iy+CopyTrackPtr)		;5120	fd 96 37 	. . 7 
	sub (iy+CopyTrackCount)		;5123	fd 96 36 	. . 6 
	jr nz,l50f1h		;5126	20 c9 	  . 
	ld a,(iy+CopyTrackCount)		;5128	fd 7e 36 	. ~ 6 
	add a,(iy+CopyTrackPtr)		;512b	fd 86 37 	. . 7 
	ld (iy+CopyTrackPtr),a		;512e	fd 77 37 	. w 7 
	call PowerOff		; Disk power OFF		;5131	cd 52 5f 	. R _ 
	jp l5083h		;5134	c3 83 50 	. . P 
l5137h:
	call PowerOff		; Disk power OFF		;5137	cd 52 5f 	. R _ 
...
; -- select drive D1
	ld (iy+SelectedDrive),$10		; select D1 drive												;5159	fd 36 0b 10 	. 6 . . 
	ld (iy+039h),0		;515d	fd 36 39 00 	. 6 9 . 
	ret								; --------------------- End of Proc ---------------------------	;5161	c9 	. 


l5162h:
	call ClearBASIC		;5162	cd 44 51 	. D Q 
	jp DOSError		; Error handling routine	;5165	c3 41 42 	. A B 
sub_5168h:
	...
	and 003h		;5175	e6 03 	. . 
	ld (iy+CopySrc),a		;5177	fd 77 0d 	. w . 
	...
	and 003h		;5187	e6 03 	. . 
	ld (iy+CopyDst),a		;5189	fd 77 10 	. w . 
...
l51aeh:
	...
	jp DOSError		; Error handling routine	;51c8	c3 41 42 	. A B 
l51cbh:
...
l51dah:
	push bc			;51da	c5 	. 
	ld bc,00064h		;51db	01 64 00 	. d . 
	call DELAYMS		; Delay mS in reg C								;51de	cd be 5e 	. . ^ 
	pop bc			;51e1	c1 	. 
l51e2h:
...
sub_5219h:
	ld a,(iy+CopySrc)		;5219	fd 7e 0d 	. ~ . 
	call sub_4d84h		;521c	cd 84 4d 	. . M 
	cp (iy+CopyDst)		;521f	fd be 10 	. . . 
	ret nz			;5222	c0 	. 
...
l5252h:
	...
	jp DOSError		; Error handling routine	;5265	c3 41 42 	. A B 
l5268h:
	...
	ret			;5274	c9 	. 
sub_5275h:
	ld a,(iy+CopyDst)		;5275	fd 7e 10 	. ~ . 
	call sub_4d84h		;5278	cd 84 4d 	. . M 
	cp (iy+CopySrc)		;527b	fd be 0d 	. . . 
	ret nz			;527e	c0 	. 
...
DCmdSTATUS:
	push hl			;52d5	e5 	. 
	di			;52d6	f3 	. 
	call PowerOn		; Disk power ON			;52d7	cd 41 5f 	. A _ 
	push bc			;52da	c5 	. 
	ld bc,00032h		;52db	01 32 00 	. 2 . 
	call DELAYMS		; Delay mS in reg C								;52de	cd be 5e 	. . ^ 
	pop bc			;52e1	c1 	. 
	ld (iy+UserTrackNo),000h		;52e2	fd 36 12 00 	. 6 . . 
	ld (iy+UserSectorNo),00fh		;52e6	fd 36 11 0f 	. 6 . . 
	call READSECTOR		; Read a sector from disk						;52ea	cd 27 5b 	. ' [ 
	or a			;52ed	b7 	. 
	jp nz,DOSError		; Error handling routine	;52ee	c2 41 42 	. A B 
	call PowerOff		; Disk power OFF		;52f1	cd 52 5f 	. R _ 
	ld l,(iy+DataBuf)		;52f4	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)		;52f7	fd 66 32 	. f 2 
...
CSI:
	...
	jp nz,DOSError					; no - jump to Error handling routine -------------------------	;536b	c2 41 42 	. A B 
...
.next:
	ld (iy+Filename),' '			; store space char into Filename Buffor							;537c	fd 36 01 20 	. 6 .   
...
.testNext:
	...
	jr z,.testNext					; yes - keep skipping											;538a	28 fa 	( . 
; -- char other than ' '
...
.copyNext:
	...
	ld (iy+Filename),a				; store filename char in DOS Filename Buffer					;53a1	fd 77 01 	. w . 
...
; IN: (iy+UserTrackNo) - requested track
;     (iy+UserSectorNo)	- requested sector	
...
ReadIDAddressMark:
	...
	jr RAM_SetTrack					; set requested Track on Drive and start read					;53ee	18 0b 	. . 
ResetTrackTo0:
	ld l,10							; set Try Counter - try to read 10 times						;53f0	2e 0a 	. . 
	ld (iy+StepPhase),%00010001		; reset Step Motor lines to 0001-0001 							;53f2	fd 36 38 11 	. 6 8 . 
; -- move Head to Track 00
	ld b,40							; max 40 tracks to Step Out										;53f6	06 28 	. ( 
	call STEPOUT					; Track step out												;53f8	cd 01 5f 	. . _ 


;***************************************************************************************************
; Setup Floppy Drive to Read requested Track
RAM_SetTrack:
	ld a,(iy+UserTrackNo)			; a - requested track number									;53fb	fd 7e 12 	. ~ . 
	sub (iy+CurTrackNo)				; calculate difference (tracks to step in/out)					;53fe	fd 96 14 	. . . 
	jr z,RAM_TrackIsSet				; no difference - track is already set							;5401	28 1a 	( . 
	jp p,RAM_NeedStepIn				; reqested track is greater - need to Step In 					;5403	f2 11 54 	. . T 
; -- requested track is less than current - need to Step Out
	...
	call STEPOUT					; move R/W Head on drive to req track							;5409	cd 01 5f 	. . _ 
	jr RAM_TrackIsSet				; track is set - calculate T+S checksum and start read	 		;540c	18 0f 	. . 
RAM_BreakExit:
...
; -- reqested track is greater than current - need to Step In
	...
	call STEPIN						; move R/W Head on drive to req track							;5412	cd ce 5e 	. . ^ 
; -- wait delay 100 ms
	push bc							; save bc														;5415	c5 	. 
	ld bc,100						; 100 ms to delay												;5416	01 64 00 	. d . 
	call DELAYMS					; wait 100 ms delay												;5419	cd be 5e 	. . ^ 
...
; -- calculate expected checksum (track number + sector number)
	ld a,(iy+UserTrackNo)			; a - track number												;541f	fd 7e 12 	. ~ . 
	add a,(iy+UserSectorNo)			; add - sector number											;5422	fd 86 11 	. . . 
...
; -- wait for FDC Clock Pulse
	...
	jp p,RAM_Wait					; wait until Clock BIt = 1										;542a	f2 28 54 	. ( T 
...
; -- test if BREAK key is pressed
	...
	jr z,RAM_BreakExit				; yes - exit reading											;5436	28 d6 	( . 
...
; -- test if we have $80 received
	...
	jp nz,RAM_WaitFor80				; no - keep reding until $80 received (or BREAK key pressed)	;5443	c2 31 54 	. 1 T 
...
; -- We have 1 byte read - if 80 then still reading GAP1
	...
	jp z,RAM_ReadByte				; yes - read next byte from disk								;54c8	ca 46 54 	. F T 
...
; -- We have 1st byte read (expected $fe)
	...
	jp nz,RAM_WaitFor80				; no - start over and find byte = $80							;554d	c2 31 54 	. 1 T 

; -- first byte of IDAM received - read next byte ($e7)
...
; -- We have 2nd byte read (expected $e7)
	...
	jp nz,RAM_WaitFor80				; no - start over and find byte = $80							;55d2	c2 31 54 	. 1 T 

; -- second byte of IDAM received - read next byte ($18)
...
; -- We have 3rd byte read (expected $18)
	...
	jp nz,RAM_WaitFor80				; no - start over and find byte = $80							;5657	c2 31 54 	. 1 T 

; -- third byte of IDAM received - read next byte ($c3)
...
; -- We have 4th byte read (expected $c3)
	...
	jp nz,RAM_WaitFor80				; no - start over and find byte = $80							;56dc	c2 31 54 	. 1 T 

; -- fourth byte of IDAM received - read next byte (track number)
...
; -- wait for FDC Clock Pulse
	...
	ld a,(iy+UserTrackNo)			; a - requested track number 									;574d	fd 7e 12 	. ~ . 
...
; -- We have Track number byte read (expected the same as requested)
	...
	cp b							; is the same as requested (in IY+UserTrackNo)					;5761	b8 	. 
	jp z,RAM_ReadIDAM_SecNo			; yes - continue to read Sector number 							;5762	ca 6c 57 	. l W 

; -- wrong track number

	dec l							; decrement Try Counter - is it 0?								;5765	2d 	- 
	jp nz,RAM_SetTrack				; no - set Track on Drive and start again						;5766	c2 fb 53 	. . S 
...
; -- wait for FDC Clock Pulse
	...
	ld a,(iy+UserSectorNo)			; a - requested sector number 									;57db	fd 7e 11 	. ~ . 
...
; -- We have Sector Number byte read (expected the same as requested)
	...
	cp b							; is the same as requested (in IY+UserSectorNo)					;57ef	b8 	. 
	jp z,RAM_ReadIDAM_Crc			; yes - continue to read Checksum Byte 							;57f0	ca fb 57 	. . W 

; -- wrong Sector Number

	dec h							; decrement Sector Try Counter - is it 0?						;57f3	25 	% 
	jp nz,RAM_WaitFor80				; no - start over and read next sector on this track			;57f4	c2 31 54 	. 1 T 
...
; -- We have IDAM Crc read (expected the same as calculated before for this track and sector)
	...
	jp nz,RAM_WaitFor80				; no - start over and read again								;5876	c2 31 54 	. 1 T 
...
; IN: (iy+Filename) - name of file to create
;     (iy+FileType) - type of file to create ('T','B', etc)
;     Interrupt disabled
; OUT: a - Error code
;     (iy+NextTrackNo) - Track where is 1st sector for data
;     (iy+NextSectorNo) - 1st sector for data
;***************************************************************************************************
CREATEDIRENTRY:
; -- check if requested fire already exists
	call FINDFILE					; Search for file in Disk Directory 							;587b	cd 13 59 	. . Y 
...
; -- check if there is enough space for new Entry in Directory
	call FINDFREESPACE				; Search empty space in Directory								;5884	cd 68 59 	. h Y 
...
; -- hl will point to address in Data Buffer with empty space for new Directory Entry
	ld d,(iy+UserSectorNo)			; d - Sector Number with empty Directory Entry					;5889	fd 56 11 	. V . 
	push de							; save de - Sector number										;588c	d5 	. 
	push hl							; save hl - address in Data Buffer								;588d	e5 	. 
; -- find first not used Sector on Disk
	call FINDEMPTYSEC				; Search for empty sector in Map and mark it as used (allocate)	;588e	cd bf 58 	. . X 
...
; -- read Sector with empty space for new Directory Entry
	ld (iy+UserSectorNo),d			; set Sector Number (track is still 0)							;5895	fd 72 11 	. r . 
	push hl							; save hl - address in Data Buffer of new Directory Entry		;5898	e5 	. 
	call READSECTOR					; Read a sector from disk into Data Buffer						;5899	cd 27 5b 	. ' [ 
...
; -- store File Type
	ld a,(iy+FileType)				; a - File Type specified by user								;58a0	fd 7e 09 	. ~ . 
...
; -- store file start Track number
	ld a,(iy+NextTrackNo)			; a - Track number 												;58b2	fd 7e 16 	. ~ . 
...
; -- store file start Sector number
	...
	ld a,(iy+NextSectorNo)			; a - Sector number												;58b7	fd 7e 15 	. ~ . 
	ld (de),a						; store as 11th byte in Directory Entry							;58ba	12 	. 
	call WRITESECTOR				; Write back sector with Directory Entries to disk				;58bb	cd a1 59 	. . Y 
	ret								; ---------- End of Proc (with Error from WRITESECTOR) --------	;58be	c9 	. 


;***************************************************************************************************
; Search for an empty sector in the track map.
; IN: (iy+MapAddress) - address of Disk Map data
; OUT: a - Error Code
;      hl - address in Disk Map data of byte bitmap for empty sector
;      (iy+NextSectorNo) - number of first not used Serctor
;      (iy+NextTrackNo) - Track number with first not used Serctor
;     
;***************************************************************************************************
FINDEMPTYSEC:
; -- setup starting point
	ld (iy+NextTrackNo),1			; start search from Track 1										;58bf	fd 36 16 01 	. 6 . . 
	ld (iy+NextSectorNo),0			; start search from Sector 0									;58c3	fd 36 15 00 	. 6 . . 
	ld l,(iy+MapAddress)																			;58c7	fd 6e 34 	. n 4 
	ld h,(iy+MapAddress+1)			; hl - address of Disk Map data									;58ca	fd 66 35 	. f 5 
...
; -- current Sector is used - get next sector number
	inc (iy+NextSectorNo)			; increment Sector number										;58d4	fd 34 15 	. 4 . 
	ld a,(iy+NextSectorNo)			; a - sector number												;58d7	fd 7e 15 	. ~ . 
...
; -- current Sector is used - get next sector number
	inc (iy+NextSectorNo)			; increment Sector number										;58e4	fd 34 15 	. 4 . 
	ld a,(iy+NextSectorNo)			; a - sector number												;58e7	fd 7e 15 	. ~ . 
...
; -- 16 sectors checked (whole track) - set sector 0 on next track 
	ld (iy+NextSectorNo),0			; set Sector number 0											;58ee	fd 36 15 00 	. 6 . . 
	inc (iy+NextTrackNo)			; increment Track number										;58f2	fd 34 16 	. 4 . 
	ld a,(iy+NextTrackNo)			; a - Track number												;58f5	fd 7e 16 	. ~ . 
...
; -- restore place and order of bits in Map Byte  
	ld a,(iy+NextSectorNo)			; a - current Sector number										;5903	fd 7e 15 	. ~ . 
...
;     (iy+Filename) - filename of file to find
; OUT: a - Error code
...
; -- setup starting condition: data buffer, track and sector number 
	ld l,(iy+DataBuf)				; hl - address of DOS Data Buffer								;5913	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)																				;5916	fd 66 32 	. f 2 
	ld (iy+UserTrackNo),0			; search will start from Track Number 0							;5919	fd 36 12 00 	. 6 . . 
	ld (iy+UserSectorNo),0			; search will start from Sector Number 0						;591d	fd 36 11 00 	. 6 . . 

; -- read Sector from Disk and compare all directory entries
.readNextSector:
	call READSECTOR					; Read a sector from disk into iy+SectorBuffer					;5921	cd 27 5b 	. ' [ 
	or a							; was any Error? 												;5924	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine							;5925	c2 41 42 	. A B 

; -- we have sector with disk directory entries - read Directory Entry
	ld b,8							; b - 8 Directory Entries in 1 Sector							;5928	06 08 	. . 
	ld l,(iy+DataBuf)																				;592a	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)				; hl - address of Data Buffer with 1st directory entry			;592d	fd 66 32 	. f 2 
...
; -- filetype other than 0 or 1
	ld (iy+FileType+1),a			; store filetype in filetype field of DOS structure (2nd byte)	;593d	fd 77 0a 	. w . 
...
; -- all 8 entries compared - read next Sector 
	inc (iy+UserSectorNo)			; increment Sector number (track still 0) to read				;595b	fd 34 11 	. 4 . 
	ld a,(iy+UserSectorNo)			; a - next sector number										;595e	fd 7e 11 	. ~ . 
...
;      (iy+UserSectorNo) - sector with empty entry (already loaded to Data Buf)
;      (iy+UserTrackNo) - track where is sector with empty entry (already loaded to Data Buf)
;***************************************************************************************************
FINDFREESPACE:
; -- setup starting condition: data buffer, track and sector number 
	ld l,(iy+DataBuf)				; hl - address of DOS Data Buffer								;5968	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)																				;596b	fd 66 32 	. f 2 
	ld (iy+UserTrackNo),0			; search will start from Track Number 0							;596e	fd 36 12 00 	. 6 . . 
	ld (iy+UserSectorNo),0			; search will start from Sector Number 0						;5972	fd 36 11 00 	. 6 . . 

; -- read Sector from Disk and find first empty place for directory entry
.readNextSector:
	call READSECTOR					; Read a sector from disk into iy+SectorBuffer					;5976	cd 27 5b 	. ' [ 
	or a							; was any Error? 												;5979	b7 	. 
	jp nz,DOSError					; yes - go to Error handling routine							;597a	c2 41 42 	. A B 

; -- we have sector with disk directory entries - read Directory Entry
	ld b,8							; b - 8 Directory Entries in 1 Sector							;597d	06 08 	. . 
	ld l,(iy+DataBuf)																				;597f	fd 6e 31 	. n 1 
	ld h,(iy+DataBuf+1)				; hl - address of Data Buffer with 1st directory/file entry		;5982	fd 66 32 	. f 2 
.checkDirEntry:
...
; -- all 8 entries checked - read next Sector 
	inc (iy+UserSectorNo)			; increment Sector number (track still 0) to read				;5994	fd 34 11 	. 4 . 
	ld a,(iy+UserSectorNo)			; a - next sector number										;5997	fd 7e 11 	. ~ . 
...
;***************************************************************************************************
; Write the content of the buffer pointed to by iy+DataBuf to the track and sector 
; specified by user.
; IN: (iy+UserTrackNo) - track number
;     (iy+UserSeckNo) - sector number
;     (iy+DataBuf) - address of 128 bytes buffer with data to write
; OUT: a - Error code
;***************************************************************************************************
WRITESECTOR:
; -- calculate CRC
...
; -- store Checksum into Sector Buffer  
	...
	ld de,SectorBuffer+128			; de - offset where CRC to be stored (just after sector data)	;59a8	11 cd 00 	. . . 
...
; -- find sector to write to and set ready to write data
	...
	call ReadIDAddressMark			; Read identification address mark (Disk head on start sector)	;59ba	cd ea 53 	. . S 
...
; -- set Write Request bit in hardware Floppy Control Register
	ld d,(iy+FLCtrlShadow)			; d - value last sent to FLCtrl									;59c7	fd 56 33 	. V 3 
...
.nextByte:
	...
	jp nz,WS_WriteByte			; continue to write all 140 bytes do Disk						;5b03	c2 cf 59 	. . Y 
...
; -- reset Wriite Request bit and update FLCtrl shadow
	...
	ld (iy+FLCtrlShadow),a			; store FLCtrl to shadow register								;5b0b	fd 77 33 	. w 3 
...
; IN: (iy+UserTrackNo) - Track number to read
;     (iy+UserSectorNo) - Sector number to read
;	  (iy+DataBuf) - address of 128 byte buffer to store sector
;     interrupt disabled
; OUT: a - Error Code
;***************************************************************************************************
READSECTOR:
; -- set number of tries
	ld (iy+RetryCount),10			; try to read sector max 10 times								;5b27	fd 36 13 0a 	. 6 . . 
RS_StartRead:
	call ReadIDAddressMark			; Read IDAM header												;5b2b	cd ea 53 	. . S 
	jp z,ReadSectorData				; if 0 (No Error) continue to read Sector Data					;5b2e	ca 37 5b 	. 7 [ 
...
; -- set destination address to store Sector data of destination 
	...
	add hl,de						; hl - points to bute just before Sector Buffer memory			;5b3d	19 	. 
; -- set number of bytes to read - 128 bytes Sector data, 2 bytes Data Checksum, 1 - ???
	...
	jr RS_Wait						; wait for Clock Bit and synchronize to byte 80					;5b40	18 03 	. . 
RS_BreakExit:
...
; -- test if BREAK key is pressed
	...
	jr z,RS_BreakExit				; yes - exit reading											;5b55	28 eb 	( . 
...
; -- wait for FDC Clock Pulse
	...
	jp p,.waitClockBit1				; wait until Clock BIt = 1										;5b5c	f2 5a 5b 	. Z [ 
; -- test if we have $80 received
	ld a,b							; byte being received											;5b5f	78 	x 
	cp $80							; is this $80 - GAP2 start sequence								;5b60	fe 80 	. . 
	jp nz,RS_WaitFor80				; no - keep reding until $80 received (or BREAK key pressed)	;5b62	c2 50 5b 	. P [ 
...
; -- We have 1 byte read - if 80 then still reading GAP2
	...
	jp z,RS_ReadByte				; yes - read next byte from disk								;5be7	ca 65 5b 	. e [ 
...
; -- We have 1st byte read (expected $c3)
	...
	jp nz,RS_WaitFor80				; no - start over and find byte = $80							;5c6c	c2 50 5b 	. P [ 

; -- first byte of IDAM received - read next byte ($18)
...
; -- We have 2nd byte read (expected $18)
	...
	jp nz,READSECTOR				; no - start over and Read a sector from disk					;5cf1	c2 27 5b 	. ' [ 

; -- second byte of IDAM received - read next byte ($e7)	
...
; -- We have 3rd byte read (expected $e7)
	...
	jp nz,READSECTOR				; no - start over and Read a sector from disk					;5d76	c2 27 5b 	. ' [ 

; -- third byte of IDAM received - read next byte ($fe)
...
; -- We have 4th byte read (expected $fe)
	...
	jp nz,READSECTOR				; no - start over and  Read a sector from disk					;5dfb	c2 27 5b 	. ' [ 

; -- fourth byte of IDAM received - read next byte will be Sector Data Byte
...
; -- We have Data byte from Disk
	...
	jr z,RS_VerifyCRC				; if all read verify Sector Data Checksum						;5e7f	28 07 	( . 
...
; -- store Data Byte into Sector Buffer and read next byte
	...
	jp RS_ReadNext					; start read next Data Byte										;5e85	c3 05 5e 	. . ^ 
RS_VerifyCRC:
	...
	jr nz,RS_ErrTryAgain			; no - try read again or exit with Error 10 - CHECKSUM ERROR	;5e8e	20 05 	  . 
...
RS_ErrTryAgain:
	ld a,(iy+RetryCount)			; a - number of read tries when CRC error 						;5e95	fd 7e 13 	. ~ . 
	dec a							; decrement Try Counter	- is it already 0?						;5e98	3d 	= 
	ld (iy+RetryCount),a			; store back number of tries									;5e99	fd 77 13 	. w . 
	jp nz,RS_StartRead				; no - start Read Sector again									;5e9c	c2 2b 5b 	. + [ 
...
; -- wait 20 ms
	...
	call DELAYMS					; wait 20 ms delay												;5eaf	cd be 5e 	. . ^ 
...
; -- decrement number of miliseconds of wait
	...
	jr nz,DELAYMS					; no - wait another milisecond									;5ecb	20 f1 	  . 
...
; -- calculate maximum number of tracks we can step in from current
	ld a,(iy+CurTrackNo)			; a - current Track Number										;5ece	fd 7e 14 	. ~ . 
...
.continue:
	ld (iy+CurTrackNo),a			; set final number as current track number						;5ed8	fd 77 14 	. w . 
	sla b							; b - physical steps needed (b * 2 steps per Track)				;5edb	cb 20 	.   

.doStepIn:
; -- calculate and store new Step Phase 
	ld a,(iy+StepPhase)				; a - current Step Phase										;5edd	fd 7e 38 	. ~ 8 
...
; -- example: when old Phase was 0001 and new Phase is 0010 then combined Phase is 0011
	...
	ld (iy+StepPhase),a				; set new value as current Step Phase							;5ee3	fd 77 38 	. w 8 
...
; -- delay 2 ms
	...
	call DELAYMS					; delay 2 ms 													;5eee	cd be 5e 	. . ^ 
...
; -- delay 14 ms
	...
	call DELAYMS					; delay 14 ms													;5efa	cd be 5e 	. . ^ 
...
; -- calculate maximum number of tracks we can step out from current
	ld a,(iy+CurTrackNo)			; a - current Track Number										;5f01	fd 7e 14 	. ~ . 
...
.continue:
	ld (iy+CurTrackNo),a			; set final number as current track number						;5f09	fd 77 14 	. w . 
	sla b							; b - physical steps needed (b * 2 steps per Track)				;5f0c	cb 20 	.   

.doStepOut:
; -- calculate and store new Step Phase 
	ld a,(iy+StepPhase)				; a - current Step Phase										;5f0e	fd 7e 38 	. ~ 8 
...
; -- example: when old Phase was 0100 and new Phase is 0010 then combined Phase is 0110
	...
	ld (iy+StepPhase),a				; set new value as current Step Phase							;5f14	fd 77 38 	. w 8 
...
; -- delay 2 ms
	...
	call DELAYMS					; delay 2 ms 													;5f1f	cd be 5e 	. . ^ 
...
; -- write Step Phase
	...
	call DELAYMS					; delay 14 ms 													;5f2b	cd be 5e 	. . ^ 
...
WriteStepPhase:
	...
	and (iy+FLCtrlShadow)			; a - FLCtrl last value w/o Step Phase bits						;5f37	fd a6 33 	. . 3 
	or c							; a - add new Step Phase bits									;5f3a	b1 	. 
	ld (iy+FLCtrlShadow),a			; store FLCtrl to shadow register								;5f3b	fd 77 33 	. w 3 
...
;***************************************************************************************************
; Disk power ON
; Turn ON the power of the drive selected in DOS vector IY+SelectedDrive. 
; IN: IY+SelectedDrive: $10 - Drive 1, $80 - Drive 2
; OUT: ---
; Registers affected: A
;***************************************************************************************************
PowerOn:
; -- get current state of FDD Step Motor
	ld a,(iy+StepPhase)				; get current Step Motor Phase									;5f41	fd 7e 38 	. ~ 8 
	and FL_STEPPER_MASK				; only 4 low bits												;5f44	e6 0f 	. . 
; -- add bits sent before to Floppy Drive Control Register 
	or (iy+FLCtrlShadow)			; add last byte sent to FLCtrl 									;5f46	fd b6 33 	. . 3 
; -- select drive
	or (iy+SelectedDrive)			; set bit for drive 											;5f49	fd b6 0b 	. . . 
; -- write new parameters to Control register and it's shadow
	ld (iy+FLCtrlShadow),a			; store FLCtrl to shadow register								;5f4c	fd 77 33 	. w 3 
...
; Both disks are turned OFF with the write request line set high at the same time.
;***************************************************************************************************
PowerOff:
; -- get bits sent before to Floppy Drive Control Register 
	ld a,(iy+FLCtrlShadow)			; get last byte sent to FLCtrl 									;5f52	fd 7e 33 	. ~ 3 
; -- clear WriteReq bit (deactivate) 
	or FL_WRITE_REQ					; set bit 6 - Write Request (active LOW)						;5f55	f6 40 	. @ 
; -- disable drive D1 and D2 and set Step MOtor to 0-0-0-0
	and %01100000					; clear all bits except WriteReq and Write Data					;5f57	e6 60 	. ` 
; -- write new parameters to Control register and it's shadow
	ld (iy+FLCtrlShadow),a			; store FLCtrl to shadow register								;5f59	fd 77 33 	. w 3 
